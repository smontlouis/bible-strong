...
| Resource publication bundle | Versioned, validated handoff produced by Resource Studio for exactly one Resource identity and Resource revision. It contains a canonical import representation, the matching Offline-copy artifact, manifest, provenance, rights, and integrity metadata. | Resource Studio and Resource publication pipeline |
...
- Resource Studio owns generation and validation of a Resource publication bundle; the Resource service owns importing that bundle into its canonical database and serving it through the Resource domain API.
- Each Resource publication bundle is independently importable. Its format version is owned by Resource Studio, and the Resource service accepts only explicitly supported versions.
