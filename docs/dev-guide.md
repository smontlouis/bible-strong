...
from the dedicated TanStack Start application under `apps/site`.
...
Environment files belong to the application that consumes them. Expo development, staging, and production files live under `apps/expo/`; other apps document their own environment contracts in their README files.

```bash
cp apps/expo/.env.example apps/expo/.env.development
```

Do not commit secrets. Expo variables used by client code must use the `EXPO_PUBLIC_*` prefix. Treat Firebase files, bundle identifiers, package names, and EAS profiles as sensitive.
