    ...
    (err, data) => {
      ...
      try {
        ...
        output.on('close', async () => {
          ...
          const releaseUrl = updatesUrl.toString().replace('/manifest', '')

          console.log(
            chalk.blue('expo-up: ') + 'Detected Release URL: ' + releaseUrl
          )
          ...
          try {
            const { status } = await axios.post(releaseUrl, formData, {
              headers: {
                ...formData.getHeaders(),
