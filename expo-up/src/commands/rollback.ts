  ...
  figlet.text(
    ...
    async (err, data) => {
      ...
      try {
        ...
        gettingAppInfoSpinner.succeed('Successfully retrieved app information.')
        ...
        const updatesUrl = new URL(exp.updates.url)
        const rollbackUrl = updatesUrl.toString().replace('/manifest', '')

        console.log(chalk.blue('expo-up: ') + 'Detected Host: ' + rollbackUrl)
