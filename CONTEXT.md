...
| Initial offline setup | One-time onboarding step that builds the device's initial Offline-copy selection. It always includes the language-specific Required startup Bible and offers thematic Offline setup folders in which the user can select additional resources. | Onboarding and resource installation |
| Required startup Bible | Language-specific Bible Offline copy required to initialize and operate the reader: LSG for French and KJV for English. It is preselected and locked during Initial offline setup; the user may add other Bibles but cannot replace or remove it. Its Strong index remains optional. | Initial offline setup and Bible reader initialization |
...
| Initial offline selection | Mutable onboarding draft composed of the Required startup Bible and the additional Offline copies selected across Offline setup folders. It is deduplicated before its missing copies enter the download queue, then ceases to exist as a product object. | Initial offline setup |
...
- Initial offline setup always includes its language-specific Required startup Bible. LSG cannot be removed or replaced in French, and KJV cannot be removed or replaced in English; their optional Strong indexes are separate Offline copies.
