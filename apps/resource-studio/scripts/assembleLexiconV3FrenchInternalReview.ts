...
import {
  ...
  type FrenchInternalArbitration,
...
export const FRENCH_INTERNAL_PROPOSER_ARTIFACT_SCHEMA_VERSION =
  "lexicon-v3-french-internal-proposer@3" as const;
export const FRENCH_INTERNAL_ARBITER_ARTIFACT_SCHEMA_VERSION =
  "lexicon-v3-french-internal-arbiter@2" as const;
export const FRENCH_INTERNAL_AUDITOR_ARTIFACT_SCHEMA_VERSION =
  "lexicon-v3-french-internal-auditor@2" as const;
export const FRENCH_INTERNAL_ASSEMBLY_CONFIG_SCHEMA_VERSION =
  "lexicon-v3-french-internal-assembly-config@2" as const;
export const FRENCH_INTERNAL_ASSEMBLY_SUMMARY_SCHEMA_VERSION =
  "lexicon-v3-french-internal-assembly-summary@5" as const;

const DEFAULT_BATCH_MANIFEST =
  "outputs/lexicon-v3/fr-internal/agent-batches/full/manifest.json";
...
const DEFAULT_ENTITY_PACKETS =
  "outputs/lexicon-v3/fr-internal/french-packets.jsonl";
const DEFAULT_EXECUTION_RECEIPTS =
  "outputs/lexicon-v3/fr-internal/full/execution-receipts.jsonl";
const DEFAULT_EXECUTION_RECEIPTS_SUMMARY =
  "outputs/lexicon-v3/fr-internal/full/execution-receipts.summary.json";
const DEFAULT_ADJUDICATION_SUMMARY =
  "outputs/lexicon-v3/fr-internal/full/adjudication-summary.json";
const DEFAULT_OUTPUT = "outputs/lexicon-v3/fr-internal/french-review.jsonl";
const DEFAULT_SUMMARY =
  "outputs/lexicon-v3/fr-internal/french-review.summary.json";
const SHA256_PATTERN = /^[a-f0-9]{64}$/u;

export interface FrenchInternalProposerArtifact {
  schemaVersion: typeof FRENCH_INTERNAL_PROPOSER_ARTIFACT_SCHEMA_VERSION;
...
export interface FrenchInternalArbiterArtifact {
  schemaVersion: typeof FRENCH_INTERNAL_ARBITER_ARTIFACT_SCHEMA_VERSION;
  role: "arbiter";
  entryKey: string;
  packetHash: string;
  englishHash: string;
...
export interface FrenchInternalAuditorArtifact {
  schemaVersion: typeof FRENCH_INTERNAL_AUDITOR_ARTIFACT_SCHEMA_VERSION;
  role: "auditor";
  entryKey: string;
  packetHash: string;
  englishHash: string;
...
export type FrenchInternalRoleArtifact =
  | FrenchInternalProposerArtifact
  | FrenchInternalArbiterArtifact
  | FrenchInternalAuditorArtifact;

export interface FrenchInternalAssemblyConfigurationFile {
  schemaVersion: typeof FRENCH_INTERNAL_ASSEMBLY_CONFIG_SCHEMA_VERSION;
  configuration: FrenchInternalReviewConfiguration;
  generationConfigHash: string;
}

export interface FrenchInternalAssemblyInputs {
  packets: LexiconV3FrenchPacket[];
  proposerA: FrenchInternalProposerArtifact[];
  proposerB: FrenchInternalProposerArtifact[];
  arbiters: FrenchInternalArbiterArtifact[];
  auditors: FrenchInternalAuditorArtifact[];
  configuration: FrenchInternalAssemblyConfigurationFile;
  entityMentions: FrenchEntityMentionsArtifact;
  execution: FrenchInternalVerifiedExecution;
  generatedAt?: string;
}

export interface FrenchInternalAssemblyBuild {
...
export interface AssembleFrenchInternalReviewOptions {
...
export interface FrenchInternalVerifiedExecution {
  namespace: string;
  releaseKey: string;
...
export interface FrenchInternalAssemblySummary {
  schemaVersion: typeof FRENCH_INTERNAL_ASSEMBLY_SUMMARY_SCHEMA_VERSION;
  ...
  sourcePaths: {
    ...
    executionReceiptsSummary: string;
...
export interface FrenchInternalAssemblyReplayResolution {
  summary: FrenchInternalAssemblySummary;
  reused: boolean;
}

interface JsonlRead<T> {
  records: T[];
  digest: string;
}
...
async function main(): Promise<void> {
  ...
  const summary = assembleLexiconV3FrenchInternalReview({
    ...
    entityMentionsPath: resolve(
      args["entity-mentions"] ?? DEFAULT_ENTITY_MENTIONS
    ),
    entityMentionResolutionAttestationPath: resolve(
      args["entity-mention-resolution-attestation"] ??
        DEFAULT_ENTITY_MENTION_RESOLUTION_ATTESTATION
    ),
    entityPacketsPath: resolve(
      args["entity-packets"] ?? DEFAULT_ENTITY_PACKETS
    ),
    executionReceiptsPath: resolve(
      args["execution-receipts"] ?? DEFAULT_EXECUTION_RECEIPTS
    ),
    executionReceiptsSummaryPath: resolve(
      args["execution-receipts-summary"] ?? DEFAULT_EXECUTION_RECEIPTS_SUMMARY
    ),
    adjudicationSummaryPath: resolve(
      args["adjudication-summary"] ?? DEFAULT_ADJUDICATION_SUMMARY
    ),
...
export function assembleLexiconV3FrenchInternalReview(
  options: AssembleFrenchInternalReviewOptions
  ...
  let batchManifestDigest: string | undefined;
  let batchManifestContext: FrenchCodexManifestContext | undefined;
  ...
  const canonicalEntities = readRawJsonl<FrenchCanonicalEntityRecord>(
    options.canonicalEntitiesPath,
    "canonical-entities"
  );
  const canonicalEntryPolicies = readRawJsonl<FrenchCanonicalEntryNamePolicy>(
    options.canonicalEntryPoliciesPath,
    "canonical-entry-policies"
  );
  const entityGate = readJsonFile<FrenchEntityCanonicalizationGateResult>(
    options.entityGatePath,
    "french-internal-entity-gate"
  );
  const entityMentions = readJsonFile<FrenchEntityMentionsArtifact>(
    options.entityMentionsPath,
    "french-internal-entity-mentions"
  );
  const entityMentionResolutionAttestation =
    options.entityMentionResolutionAttestationPath
      ? readJsonFile<FrenchEntityMentionResolutionAttestation>(
          options.entityMentionResolutionAttestationPath,
          "french-internal-entity-mention-resolution-attestation"
        )
      : undefined;
  const entityPackets = readFrenchInternalPackets(options.entityPacketsPath);
  const entityPacketByKey = new Map(
    entityPackets.records.map((packet) => [packet.entryKey, packet])
  );
  for (const packet of packets.records) {
    if (
      ...
      throw new Error(
        `french-internal-entity-packet-selection-drift:${packet.entryKey}`
      );
    }
  }
  ...
  const existingPair = readExistingFrenchInternalAssemblyPair(
    options.outputPath,
    options.summaryPath
  );
  const requestedGeneratedAt =
  ...
  if (
    ...
    throw new Error(
      "french-internal-existing-output-pair-generated-at-mismatch"
    );
  }
  ...
  const summaryWithoutDigest = {
    ...
    sourceDigests: {
      ...
      ...(options.entityMentionResolutionAttestationPath
        ? {
            entityMentionResolutionAttestation: sha256File(
              options.entityMentionResolutionAttestationPath
            )
          }
        : {}),
      ...
      executionReceiptsSummary: sha256File(
        options.executionReceiptsSummaryPath
      ),
  ...
  const summary: FrenchInternalAssemblySummary = {
  ...
  if (replay.reused) return replay.summary;
  writeOutputPairAtomic(
    options.outputPath,
    outputText,
    options.summaryPath,
    summaryText
...
export function frenchInternalAssemblyGeneratedAt(
  execution: FrenchInternalVerifiedExecution,
  requested?: string
): string {
  ...
  for (const [entryKey, receipts] of execution.receiptsByEntry) {
    for (const role of [
      ...
      if (!isCanonicalIsoTimestamp(value)) {
        throw new Error(
          `invalid-french-internal-receipt-completed-at:${entryKey}:${role}:${String(value)}`
        );
      }
...
export function resolveFrenchInternalAssemblyPairForReplay(input: {
  ...
  expectedSummary: FrenchInternalAssemblySummary;
  existingOutputText?: string;
  existingSummaryText?: string;
}): FrenchInternalAssemblyReplayResolution {
  ...
  assertFrenchInternalAssemblySummarySelfHash(
    input.expectedSummary,
    "french-internal-expected-output-pair-invalid"
  );
  if (!outputExists) {
    return { summary: input.expectedSummary, reused: false };
  }

  const existingSummary = parseFrenchInternalAssemblySummary(
    input.existingSummaryText!,
    "french-internal-existing-output-pair-stale"
  );
  const expectedSummaryText = `${JSON.stringify(input.expectedSummary, null, 2)}\n`;
  if (
    input.existingOutputText !== input.expectedOutputText ||
...
export function assembleFrenchInternalReviewRecords(
  input: FrenchInternalAssemblyInputs
): FrenchInternalAssemblyBuild {
  ...
  for (const [index, packet] of input.packets.entries()) {
    ...
    if (issues.length > 0) {
      throw new Error(
        `invalid-french-internal-packet:memory:${index + 1}:${issues.join(",")}`
      );
    }
  }
  assertUniquePacketIdentities(input.packets, "memory");
  input.proposerA.forEach((artifact, index) =>
    validateProposerArtifact(artifact, "proposerA", `memory:${index + 1}`)
  );
  input.proposerB.forEach((artifact, index) =>
    validateProposerArtifact(artifact, "proposerB", `memory:${index + 1}`)
  );
  input.arbiters.forEach((artifact, index) =>
    validateArbiterArtifact(artifact, `memory:${index + 1}`)
  );
  input.auditors.forEach((artifact, index) =>
    validateAuditorArtifact(artifact, `memory:${index + 1}`)
  );
  ...
  const generatedAt = frenchInternalAssemblyGeneratedAt(
    input.execution,
    input.generatedAt
  );
  ...
  const entityMentionsByEntry = groupRequiredEntityMentions(
    input.entityMentions.requiredEntityMentions
  );

  const records: FrenchInternalReviewRecord[] = [];
  for (const entryKey of [...packets.keys()].sort((left, right) =>
    ...
    if (
      hashFrenchInternalJson(roleA.requiredEntityMentions) !==
        hashFrenchInternalJson(requiredEntityMentions) ||
      hashFrenchInternalJson(roleB.requiredEntityMentions) !==
        hashFrenchInternalJson(requiredEntityMentions)
    ) {
      throw new Error(
        `french-internal-proposer-entity-lineage-drift:${entryKey}`
      );
    }
      ...
      assembleFrenchInternalEntry({
        ...
        executionAttestation: finalizeFrenchInternalExecutionAttestation({
          ...
          releaseSnapshotFingerprint:
            input.execution.releaseSnapshotFingerprint,
...
export function readFrenchInternalPackets(
  path: string
): JsonlRead<LexiconV3FrenchPacket> {
  const result = readJsonl(path, (value, lineNumber) => {
    ...
    if (issues.length > 0) {
      throw new Error(
        `invalid-french-internal-packet:${lineNumber}:${issues.join(",")}`
      );
    }
    return packet;
  });
...
export function readFrenchInternalProposerArtifacts(
  path: string,
  expectedRole: "proposerA" | "proposerB"
): JsonlRead<FrenchInternalProposerArtifact> {
  return readJsonl(path, (value, lineNumber) =>
    validateProposerArtifact(value, expectedRole, `${path}:${lineNumber}`)
  );
}

export function readFrenchInternalArbiterArtifacts(
  path: string
): JsonlRead<FrenchInternalArbiterArtifact> {
  return readJsonl(path, (value, lineNumber) =>
    validateArbiterArtifact(value, `${path}:${lineNumber}`)
  );
}

export function readFrenchInternalAuditorArtifacts(
  path: string
): JsonlRead<FrenchInternalAuditorArtifact> {
  return readJsonl(path, (value, lineNumber) =>
    validateAuditorArtifact(value, `${path}:${lineNumber}`)
  );
}

export function readFrenchInternalAssemblyConfiguration(
  path: string
): FrenchInternalAssemblyConfigurationFile {
  let value: unknown;
  try {
  ...
  const configuration = value as FrenchInternalAssemblyConfigurationFile;
 ...
 */
export function assertFrenchInternalConfigurationMatchesManifest(
  context: FrenchCodexManifestContext,
  configurationPath: string
): FrenchInternalAssemblyConfigurationFile {
  const configuration =
    readFrenchInternalAssemblyConfiguration(configurationPath);
  if (
    context.runKind === "full" &&
...
export function validateFrenchInternalAssemblyConfiguration(
  value: FrenchInternalAssemblyConfigurationFile
): void {
  assertObject(value, "invalid-french-internal-configuration");
  assertExactKeys(
    value,
    ["schemaVersion", "configuration", "generationConfigHash"],
    "invalid-french-internal-configuration-keys"
  );
  ...
  if (
    hashFrenchInternalJson(configuration.approvedExecutionProfile) !==
...
export function readFrenchInternalVerifiedExecution(input: {
  ...
  const summary = readJsonObject(
    input.receiptsSummaryPath,
    "french-internal-execution-summary-invalid-json"
  );
  ...
  const sourcePaths = assertStringRecord(
    summary.sourcePaths,
    "french-internal-execution-source-paths-invalid"
  );
  const sourceDigests = assertStringRecord(
    summary.sourceDigests,
    "french-internal-execution-source-digests-invalid"
  );
  ...
  if (
    JSON.stringify(Object.keys(sourcePaths).sort()) !==
  ...
  if (
    resolve(sourcePaths.proposerManifest!) !==
  ...
  const proposerSummary = verifySelfHashedJson(
    sourcePaths.proposerSummary!,
    "summaryHash",
    "french-internal-execution-proposer-summary-invalid"
  );
  const arbiterManifest = verifySelfHashedJson(
    sourcePaths.arbiterManifest!,
    "manifestHash",
    "french-internal-execution-arbiter-manifest-invalid"
  );
  const auditorManifest = verifySelfHashedJson(
    sourcePaths.auditorManifest!,
    "manifestHash",
    "french-internal-execution-auditor-manifest-invalid"
  );
  const arbiterSummary = verifySelfHashedJson(
    sourcePaths.arbiterSummary!,
    "summaryHash",
    "french-internal-execution-arbiter-summary-invalid"
  );
  const auditorSummary = verifySelfHashedJson(
    sourcePaths.auditorSummary!,
    "summaryHash",
    "french-internal-execution-auditor-summary-invalid"
  );
  if (
    proposerSummary.manifestHash !== manifest.manifestHash ||
  ...
  const output = assertObjectValue(
    summary.output,
    "french-internal-execution-output-invalid"
  );
  if (
    resolve(String(output.path ?? "")) !== resolve(input.receiptsPath) ||
  ...
  const packetsByEntry = new Map(
    input.packets.map((packet) => [packet.entryKey, packet])
  );
  const artifactsByRole = {
    proposerA: new Map(
      input.roleArtifacts.proposerA.map((artifact) => [
        artifact.entryKey,
        artifact
      ])
    ),
    proposerB: new Map(
      input.roleArtifacts.proposerB.map((artifact) => [
        artifact.entryKey,
        artifact
      ])
    ),
    arbiter: new Map(
      input.roleArtifacts.arbiter.map((artifact) => [
        artifact.entryKey,
        artifact
      ])
    ),
    auditor: new Map(
      input.roleArtifacts.auditor.map((artifact) => [
        artifact.entryKey,
        artifact
      ])
    )
  ...
  for (const packet of input.packets) {
    const values = receipts.filter(
      (receipt) => receipt.entryKey === packet.entryKey
    );
    const byRole = new Map(values.map((receipt) => [receipt.role, receipt]));
    if (values.length !== 4 || byRole.size !== 4) {
      throw new Error(
        `french-internal-execution-entry-coverage:${packet.entryKey}`
      );
    }
    ...
    for (const role of [
      ...
      if (
        ...
        throw new Error(
          `french-internal-execution-receipt-lineage:${packet.entryKey}:${role}`
        );
      }
      record[role] = receipt;
    }
    if (
      new Set(Object.values(record).map((receipt) => receipt.threadId)).size !==
      4
    ) {
      throw new Error(
        `french-internal-execution-thread-collision:${packet.entryKey}`
      );
    }
    receiptsByEntry.set(packet.entryKey, record);
  }
  const orphan = receipts.find(
    (receipt) => !packetsByEntry.has(receipt.entryKey)
  );
  if (orphan) {
    throw new Error(`french-internal-execution-orphan:${orphan.entryKey}`);
  }
  const adjudication = verifySelfHashedJson(
    input.adjudicationSummaryPath,
    "summaryHash",
    "french-internal-adjudication-summary-invalid"
  );
  const adjudicationOutputs = assertObjectValue(
    adjudication.outputs,
    "french-internal-adjudication-summary-outputs-invalid"
  );
  const adjudicationReceipts = assertObjectValue(
    adjudicationOutputs.executionReceipts,
    "french-internal-adjudication-receipts-missing"
  );
  const adjudicationOutput = assertObjectValue(
    adjudicationReceipts.output,
    "french-internal-adjudication-receipts-output-invalid"
  );
  if (
...
function readExecutionReceipts(path: string): FrenchInternalExecutionReceipt[] {
  const result: FrenchInternalExecutionReceipt[] = [];
  for (const [index, line] of readFileSync(path, "utf8")
    .split(/\r?\n/u)
    .entries()) {
    if (!line.trim()) continue;
    let value: FrenchInternalExecutionReceipt;
    try {
      value = JSON.parse(line) as FrenchInternalExecutionReceipt;
    } catch {
      throw new Error(
        `french-internal-execution-receipt-invalid-json:${index + 1}`
      );
    }
    try {
...
function verifyExecutionReceiptFiles(
  ...
  const run = readJsonObject(
    receipt.sourcePaths.runPointer!,
    "french-internal-execution-run-pointer-invalid"
  );
  const runHash = run.runHash;
  const runContent = { ...run };
  delete runContent.runHash;
  const runSourceHashes = assertStringRecord(
    run.sourceHashes,
    "french-internal-execution-run-source-hashes-invalid"
  );
  const runResultHashes = assertStringRecord(
    run.resultHashes,
    "french-internal-execution-run-result-hashes-invalid"
  );
  ...
  const replay = replayFrenchCodexAgentEvents(
    readFileSync(receipt.resultPaths.agentEvents!, "utf8"),
    readFileSync(receipt.resultPaths.structuredResponse!, "utf8")
  );
...
function replayFrenchCodexAgentEvents(
  eventsText: string,
  ...
  for (const line of eventsText.split(/\r?\n/u)) {
    ...
    if (event.type === "item.completed") {
      const item = assertObjectValue(
        event.item,
        "french-internal-execution-event-item-invalid"
      );
      if (
...
function normalizeExecutionMessage(value: string): string {
  return value
    .trim()
    .replace(/^```(?:json)?\s*/u, "")
    .replace(/\s*```$/u, "")
...
function verifySelfHashedJson(
  path: string,
  hashKey: "summaryHash" | "manifestHash",
  error: string
...
function readJsonObject(path: string, error: string): Record<string, unknown> {
  let value: unknown;
  try {
    value = JSON.parse(readFileSync(path, "utf8")) as unknown;
  } catch {
...
export function frenchInternalRoleArtifactHash(
  artifact:
    | Omit<FrenchInternalProposerArtifact, "artifactHash">
    | Omit<FrenchInternalArbiterArtifact, "artifactHash">
    | Omit<FrenchInternalAuditorArtifact, "artifactHash">
    | FrenchInternalRoleArtifact
): string {
  const { artifactHash: _artifactHash, ...content } =
    artifact as FrenchInternalRoleArtifact;
  void _artifactHash;
  return hashFrenchInternalJson(content);
}

export function finalizeFrenchInternalProposerArtifact(
  artifact: Omit<FrenchInternalProposerArtifact, "artifactHash">
): FrenchInternalProposerArtifact {
  return {
    ...artifact,
...
export function finalizeFrenchInternalArbiterArtifact(
  artifact: Omit<FrenchInternalArbiterArtifact, "artifactHash">
): FrenchInternalArbiterArtifact {
  return {
    ...artifact,
    artifactHash: frenchInternalRoleArtifactHash(artifact)
...
export function finalizeFrenchInternalAuditorArtifact(
  artifact: Omit<FrenchInternalAuditorArtifact, "artifactHash">
): FrenchInternalAuditorArtifact {
  return {
    ...artifact,
    artifactHash: frenchInternalRoleArtifactHash(artifact)
...
function buildFrenchInternalSiblingProofs(input: {
  ...
  string,
  ReturnType<typeof finalizeFrenchInternalSiblingConsistencyProof>
> {
  ...
  for (const [familyKey, membersUnsorted] of families) {
    ...
    const translations = members.map((packet) => {
      ...
      const selected =
        arbiter.selectedProposal === "proposalA"
          ? input.proposerA.get(packet.entryKey)!
          : input.proposerB.get(packet.entryKey)!;
      const proposal = renderProposerArtifact(
        selected,
        packet,
    ...
    const familyInput = translations.map(
      ({ packet, proposal, selectedArtifactHash }) => ({
    ...
    const issuesByEntry = new Map<string, string[]>();
    for (const member of members) issuesByEntry.set(member.entryKey, []);
    for (let leftIndex = 0; leftIndex < translations.length; leftIndex += 1) {
      for (
        ...
        if (
          normalizeSiblingText(left.packet.english.gloss) ===
        ...
        if (
          normalizeSiblingText(left.packet.english.meaning) ===
...
function normalizeSiblingText(value: string): string {
  return value
    .normalize("NFKC")
...
function assembleFrenchInternalEntry(input: {
  ...
  siblingConsistency: ReturnType<
    typeof finalizeFrenchInternalSiblingConsistencyProof
  >;
  requiredEntityMentions: RequiredFrenchEntityMention[];
  configuration: FrenchInternalAssemblyConfigurationFile;
  generatedAt: string;
  ...
  const proposalA = renderProposerArtifact(
    input.proposerA,
    input.packet,
    template
  );
  const proposalB = renderProposerArtifact(
    input.proposerB,
    input.packet,
    template
  );
  ...
  const selectedProposal =
    input.arbiter.selectedProposal === "proposalA" ? proposalA : proposalB;
  const selectedValidation =
    input.arbiter.selectedProposal === "proposalA" ? validationA : validationB;
  const arbitration: FrenchInternalArbitration = {
  ...
  const proofA = buildFrenchInternalAgentProof({
    ...
    executionReceiptHash:
      input.executionAttestation.roleReceipts.proposerA.receiptHash,
    response: proposalA,
  ...
  const proofB = buildFrenchInternalAgentProof({
    ...
    executionReceiptHash:
      input.executionAttestation.roleReceipts.proposerB.receiptHash,
    response: proposalB,
  ...
  const proofArbiter = buildFrenchInternalAgentProof({
    ...
    executionReceiptHash:
      input.executionAttestation.roleReceipts.arbiter.receiptHash,
  ...
  const proofAuditor = buildFrenchInternalAgentProof({
    ...
    executionReceiptHash:
      input.executionAttestation.roleReceipts.auditor.receiptHash,
  ...
  if (!evaluation.structurallyValid) {
    throw new Error(
      `invalid-assembled-french-internal-review:${input.packet.entryKey}:${evaluation.structuralIssues.join(",")}`
    );
  }
...
function renderProposerArtifact(
  artifact: FrenchInternalProposerArtifact,
  packet: LexiconV3FrenchPacket,
  template: ReturnType<typeof buildFrenchHtmlTemplate>
...
function validateProposerArtifact(
  value: unknown,
  expectedRole: "proposerA" | "proposerB",
  locator: string
): FrenchInternalProposerArtifact {
  ...
  const artifact = value as unknown as FrenchInternalProposerArtifact;
  if (
    artifact.schemaVersion !== FRENCH_INTERNAL_PROPOSER_ARTIFACT_SCHEMA_VERSION
  ) {
  ...
  for (const segment of artifact.meaningSegmentsFr) {
    ...
    assertExactKeys(
      segment,
      ["id", "text"],
      `invalid-french-internal-segment-keys:${locator}`
    );
    if (
  ...
  if (!Array.isArray(artifact.requiredEntityMentions)) {
    throw new Error(
      `invalid-french-internal-required-entity-mentions:${locator}`
    );
  }
  ...
  for (const mention of artifact.entityMentionsFr) {
    ...
    assertExactKeys(
      mention,
      ["mentionId", "segmentId", "chosenFrenchForm"],
      `invalid-french-internal-entity-mention-keys:${locator}`
    );
    if (
  ...
  if (
    !Array.isArray(artifact.carrierTermsFr) ||
    artifact.carrierTermsFr.some(
...
function validateArbiterArtifact(
  value: unknown,
  locator: string
): FrenchInternalArbiterArtifact {
  ...
  const artifact = value as unknown as FrenchInternalArbiterArtifact;
  if (
    artifact.schemaVersion !== FRENCH_INTERNAL_ARBITER_ARTIFACT_SCHEMA_VERSION
  ) {
  ...
  validateReasons(
    artifact.reasons,
    `invalid-french-internal-arbiter-reasons:${locator}`
  );
  assertRoleArtifactHash(artifact, locator);
  return artifact;
}

function validateAuditorArtifact(
  value: unknown,
  locator: string
): FrenchInternalAuditorArtifact {
  ...
  const artifact = value as unknown as FrenchInternalAuditorArtifact;
  if (
    artifact.schemaVersion !== FRENCH_INTERNAL_AUDITOR_ARTIFACT_SCHEMA_VERSION
  ) {
  ...
  validateReasons(
    artifact.reasons,
    `invalid-french-internal-auditor-reasons:${locator}`
  );
  if (
    !Number.isFinite(artifact.confidence) ||
    artifact.confidence < 0 ||
  ...
  assertObject(
    artifact.checks,
    `invalid-french-internal-auditor-checks:${locator}`
  );
  assertExactKeys(
    artifact.checks,
    [
  ...
  if (
    Object.values(artifact.checks).some(
...
function validateRoleBase(
  artifact: FrenchInternalRoleArtifact,
  locator: string
): void {
  ...
  assertSha256(
    artifact.generationConfigHash,
    `invalid-role-generation-config-hash:${locator}`
  );
...
function assertRoleArtifactHash(
  artifact: FrenchInternalRoleArtifact,
  locator: string
): void {
  if (
...
function assertRoleLineage(
  role: FrenchInternalRoleArtifact,
  packet: LexiconV3FrenchPacket,
  generationConfigHash: string
): void {
  ...
  if (role.packetHash !== packet.packetHash) {
    throw new Error(
      `french-internal-role-packet-stale:${packet.entryKey}:${role.role}`
    );
  }
  if (role.englishHash !== packet.english.contentHash) {
    throw new Error(
      `french-internal-role-english-stale:${packet.entryKey}:${role.role}`
    );
  }
  if (role.generationConfigHash !== generationConfigHash) {
    throw new Error(
      `french-internal-role-config-stale:${packet.entryKey}:${role.role}`
    );
  }
}
...
function assertEntityArtifactsMatchConfiguration(input: {
  ...
  entityGate: FrenchEntityCanonicalizationGateResult;
  entityMentions: FrenchEntityMentionsArtifact;
  entityMentionResolutionAttestation?: FrenchEntityMentionResolutionAttestation;
  paths: {
    canonicalEntities: string;
    canonicalEntryPolicies: string;
    entityMergeAttestation: string;
  ...
  const quarantinedEntryKeys = frenchEntityQuarantinedEntryKeysFromMerge({
  ...
  assertFrenchEntityPipelineArtifacts({
    ...
    mentionResolutionAttestation: input.entityMentionResolutionAttestation,
    allowConfigurationPinnedResolution: true
  });
}

function groupRequiredEntityMentions(
...
function readRawJsonl<T>(path: string, label: string): JsonlRead<T> {
...
function readJsonl<T>(
  ...
  for (const [index, line] of bytes
    ...
    const entryKey = (record as { entryKey?: unknown }).entryKey;
    if (typeof entryKey !== "string" || !entryKey.trim()) {
      throw new Error(
        `missing-french-internal-entry-key:${path}:${lineNumber}`
      );
    }
...
function uniqueByEntry<T extends { entryKey: string }>(
  values: T[],
  label: string
): Map<string, T> {
  ...
  for (const value of values) {
    if (result.has(value.entryKey)) {
      throw new Error(
        `duplicate-french-internal-entry:${label}:${value.entryKey}`
      );
    }
...
function assertUniquePacketIdentities(
  packets: LexiconV3FrenchPacket[],
  locator: string
): void {
  ...
  for (const packet of packets) {
    if (stepEntryIds.has(packet.identity.stepEntryId)) {
      throw new Error(
        `duplicate-french-internal-step-entry-id:${locator}:${packet.identity.stepEntryId}`
      );
    }
    if (packetHashes.has(packet.packetHash)) {
      throw new Error(
        `duplicate-french-internal-packet-hash:${locator}:${packet.packetHash}`
      );
    }
...
function assertExactCoverage<T>(
  ...
  const missing = [...packets.keys()].filter(
    (entryKey) => !roles.has(entryKey)
  );
  const orphan = [...roles.keys()].filter((entryKey) => !packets.has(entryKey));
  if (missing.length > 0) {
    throw new Error(
      `missing-french-internal-role:${label}:${missing.sort().join(",")}`
    );
  }
  if (orphan.length > 0) {
    throw new Error(
      `orphan-french-internal-role:${label}:${orphan.sort().join(",")}`
    );
  }
}

function validateReasons(value: unknown, error: string): void {
  if (
    !Array.isArray(value) ||
...
function readExistingFrenchInternalAssemblyPair(
  outputPath: string,
  summaryPath: string
):
  | {
...
function parseFrenchInternalAssemblySummary(
  text: string,
  error: string
): FrenchInternalAssemblySummary {
  let value: unknown;
...
function assertFrenchInternalAssemblySummarySelfHash(
  value: unknown,
  error: string
): asserts value is FrenchInternalAssemblySummary {
  try {
    assertObject(value, error);
    if (
      value.schemaVersion !== FRENCH_INTERNAL_ASSEMBLY_SUMMARY_SCHEMA_VERSION ||
...
function emptyStatusCounts(): Record<FrenchInternalReviewStatus, number> {
  return {
    auto_validated: 0,
    review_needed: 0,
...
function withoutReviewArtifactHash(
...
function writeOutputPairAtomic(
  outputPath: string,
  output: string,
  summaryPath: string,
  summary: string
  ...
  try {
    ...
    rmSync(temporaryOutput, { force: true });
    rmSync(temporarySummary, { force: true });
    if (outputInstalled) rmSync(resolvedOutput, { force: true });
    if (outputBackedUp && existsSync(backupOutput)) {
      renameSync(backupOutput, resolvedOutput);
    }
    if (summaryInstalled) rmSync(resolvedSummary, { force: true });
...
function readFrenchCodexBatchManifest(
  path: string
): FrenchCodexAnyBatchManifest {
  if (!existsSync(path)) {
...
export function parseFrenchInternalAssemblyArgs(
