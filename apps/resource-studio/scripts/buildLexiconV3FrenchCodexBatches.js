...
import { assertSelectedPacketsMetadata, assertFrenchCodexPilotBatchManifest, FRENCH_CODEX_PILOT_BATCH_MANIFEST_SCHEMA_VERSION, frenchCodexSelectedPacketsLogicalDigest } from "./buildLexiconV3FrenchCodexPilotBatches.js";
import { FRENCH_INTERNAL_PROPOSER_DRAFT_SCHEMA_VERSION } from "../src/lexiconV3/frenchAgentDrafts.js";
import { FRENCH_INTERNAL_PROMPT_VERSION, hashFrenchInternalJson } from "../src/lexiconV3/frenchInternalReview.js";
...
import { FRENCH_INTERNAL_PROPOSER_VIEW_SCHEMA_VERSION, FRENCH_INTERNAL_SHARDS_SCHEMA_VERSION, FRENCH_INTERNAL_SHARD_ITEM_SCHEMA_VERSION, FRENCH_INTERNAL_SHARD_SCHEMA_VERSION, FRENCH_INTERNAL_WORK_POLICY_VERSION, assertProposerABlindView, frenchInternalViewHash, hashFrenchInternalWorkJson } from "../src/lexiconV3/frenchInternalWork.js";
import { validateFrenchPacket } from "../src/lexiconV3/frenchPackets.js";
export const FRENCH_CODEX_BATCH_MANIFEST_SCHEMA_VERSION = "lexicon-v3-french-codex-batches@4";
export const FRENCH_CODEX_SELECTION_PROOF_SCHEMA_VERSION = "lexicon-v3-french-codex-selection-proof@3";
...
const SAFE_NAMESPACE_PATTERN = /^\/fr-internal\/(?:pilot|full|custom\/[a-z0-9][a-z0-9._-]*)$/u;
const SIZE_ORDER = [
    "short",
    "medium",
    "long",
...
export function buildLexiconV3FrenchCodexBatches(options) {
...
function buildUnlocked(options) {
    ...
    const groups = selection.sourceKind === "shards"
    ...
    try {
        ...
        const batches = groups.map((group, index) => {
            ...
            if (shardId)
                shardParts.set(shardId, nextPart);
            return writeBatch(tempDir, resolve(options.outputDir), options.runKind, suffix, group);
        });
        const selectedPackets = writeSelectedPacketsArtifact(tempDir, resolve(options.outputDir), items.map((item) => item.packet), selection);
        const byMeaningSize = Object.fromEntries(SIZE_ORDER.map((size) => [
            size,
        ...
        const installedManifestPath = join(resolve(options.outputDir), "manifest.json");
        if (existsSync(installedManifestPath)) {
            ...
            if (!options.replaceExisting) {
                throw new Error(`french-codex-batch-output-stale:${resolve(options.outputDir)}`);
            }
        }
        if (existsSync(resolve(options.outputDir)) &&
            !existsSync(installedManifestPath) &&
            !options.replaceExisting) {
            throw new Error(`french-codex-batch-output-unattested:${resolve(options.outputDir)}`);
        }
...
function buildPilotQualityGateArtifact(options, selection) {
...
function assertPilotQualityGateArtifact(manifest, verifyFiles) {
    ...
    const gate = readFrenchPilotQualityGate(artifact.path, {
        ...
        expectedReleaseSnapshotFingerprint: manifest.lineage.releaseSnapshotFingerprint,
...
function buildSelectionProof(input) {
    ...
    let sourceLogicalDigest;
    let releaseKey;
    let releaseSnapshotFingerprint;
    let fullPlanEntries;
    let exactFullCoverage;
    let shardIds;
    let keys;
    const parsed = JSON.parse(input.selectionText);
    if (parsed.schemaVersion === FRENCH_INTERNAL_SHARDS_SCHEMA_VERSION) {
        ...
        keys = plan.shards.flatMap((shard) => shard.items.map((item) => item.entryKey));
        for (const shard of plan.shards) {
            for (const item of shard.items) {
        ...
        if (!first)
            throw new Error("french-codex-selection-view-missing:first");
    ...
    if (keys.length !== input.expectedEntries ||
        keys.length < 1 ||
        new Set(keys).size !== keys.length) {
        throw new Error(`french-codex-selection-count:${keys.length}:${input.expectedEntries}`);
    }
    for (const key of keys) {
        ...
        if (proposerA.lineage.releaseKey !== releaseKey ||
    ...
    if (input.runKind === "full") {
        ...
        if (input.proposerA.size !== keys.length ||
...
function assertSelectedItemLineage(selection, proposerA, proposerB, packet, entryKey) {
    if (proposerA.lineage.releaseKey !== selection.releaseKey ||
...
function parseShardPlan(text) {
    const plan = JSON.parse(text);
    const { contentHash, ...content } = plan;
    if (plan.schemaVersion !== FRENCH_INTERNAL_SHARDS_SCHEMA_VERSION ||
...
function assertShard(shard, plan, seenShards, seenKeys) {
    const { shardHash, ...content } = shard;
    if (shard.schemaVersion !== FRENCH_INTERNAL_SHARD_SCHEMA_VERSION ||
    ...
    for (const item of shard.items) {
        ...
        if (item.schemaVersion !== FRENCH_INTERNAL_SHARD_ITEM_SCHEMA_VERSION ||
...
function groupBySealedShards(items, shardIds, options) {
    ...
    const groups = shardIds.map((shardId) => {
        ...
        if (!group?.length)
            throw new Error(`french-codex-shard-empty:${shardId}`);
        return group;
    });
    if (groups.flat().length !== items.length ||
...
function splitSealedGroup(group, options) {
    const result = [];
    let current = [];
    let bytes = 0;
    const size = group[0].meaningSize;
    if (group.some((item) => item.meaningSize !== size)) {
        throw new Error(`french-codex-sealed-shard-mixed-size:${group[0].shardId}`);
    }
    for (const item of group) {
        const itemBytes = inputBytes(item);
        if (itemBytes > options.maxCombinedBytes) {
            throw new Error(`french-codex-sealed-item-budget-invalid:${item.entryKey}`);
        }
        if (current.length > 0 &&
    ...
    for (const batch of result)
        assertBatchBudget(batch, options);
    return result;
}
function groupCustom(items, options) {
    const result = [];
    let current = [];
    let bytes = 0;
    for (const item of items) {
    ...
    for (const group of result)
        assertBatchBudget(group, options);
    if (result.flat().length !== items.length) {
        throw new Error("french-codex-custom-batch-coverage-invalid");
    }
    return result;
}
function assertBatchBudget(items, options) {
    const size = items[0].meaningSize;
    if (items.some((item) => item.meaningSize !== size) ||
        items.length > options.maxItems[size] ||
        items.reduce((sum, item) => sum + inputBytes(item), 0) >
            options.maxCombinedBytes) {
        throw new Error(`french-codex-sealed-shard-budget-invalid:${items[0].shardId ?? "custom"}`);
    }
}
function inputBytes(item) {
    return (Buffer.byteLength(`${JSON.stringify(item.proposerA)}\n`) +
        Buffer.byteLength(`${JSON.stringify(item.proposerB)}\n`));
}
function writeBatch(tempRoot, finalRoot, runKind, suffix, items) {
    ...
    const aInput = writeFinalArtifact(physicalDir, finalDir, "proposer-a-input.jsonl", jsonl(items.map((item) => item.proposerA)));
    const bInput = writeFinalArtifact(physicalDir, finalDir, "proposer-b-input.jsonl", jsonl(items.map((item) => item.proposerB)));
    const packets = writeFinalArtifact(physicalDir, finalDir, "packets.jsonl", jsonl(items.map((item) => item.packet)));
    const aSchema = writeFinalArtifact(physicalDir, finalDir, "proposer-a-output.schema.json", `${JSON.stringify(outputSchema("proposerA", items), null, 2)}\n`);
    const bSchema = writeFinalArtifact(physicalDir, finalDir, "proposer-b-output.schema.json", `${JSON.stringify(outputSchema("proposerB", items), null, 2)}\n`);
    const content = {
        batchId,
        meaningSize,
        ...
        lineage: {
            ...
            releaseSnapshotFingerprint: items[0].proposerA.lineage.releaseSnapshotFingerprint,
...
function outputSchema(role, items) {
    const keys = items.map((item) => item.entryKey);
    const hashes = items.map((item) => role === "proposerA" ? item.proposerA.viewHash : item.proposerB.viewHash);
    const segmentIds = [
        ...new Set(items.flatMap((item) => (role === "proposerA"
            ? item.proposerA
            : item.proposerB).translationTask.htmlTemplate.tokens.flatMap((token) => token.kind === "text" && token.translatable ? [token.id] : [])))
    ];
    return {
...
export function assertFrenchCodexBatchManifest(manifest, options = {}) {
    if (manifest.schemaVersion !== FRENCH_CODEX_BATCH_MANIFEST_SCHEMA_VERSION) {
        throw new Error(`french-codex-batch-manifest-migration-required:${String(manifest.schemaVersion)}`);
    }
    const { manifestHash, ...content } = manifest;
    const selection = manifest.selection;
    const { contentHash: selectionHash, ...selectionContent } = selection;
    if (manifest.policyVersion !== FRENCH_INTERNAL_WORK_POLICY_VERSION ||
    ...
    assertSelectedPacketsMetadata(manifest.selectedPackets, join(manifest.outputRoot, "selected-packets.jsonl"), manifest.counts.entries, manifest.lineage);
    assertPilotQualityGateArtifact(manifest, options.verifyFiles === true);
    if (manifest.runKind === "full") {
        const observedShards = [];
        const nextPart = new Map();
        for (const batch of manifest.batches) {
            const match = /^full-((?:short|medium|long|very_long)-\d{4})-p(\d{3})$/u.exec(batch.batchId);
            if (!match) {
                throw new Error(`french-codex-full-batch-id-invalid:${batch.batchId}`);
            }
            const shardId = match[1];
            const part = Number(match[2]);
            if (!nextPart.has(shardId))
                observedShards.push(shardId);
        ...
        if (observedShards.length !== selection.shardIds.length ||
            observedShards.some((shardId, index) => shardId !== selection.shardIds[index])) {
            throw new Error("french-codex-full-shard-coverage-invalid");
        }
    }
    else if (manifest.batches.some((batch) => !/^custom-(?:short|medium|long|very_long)-\d{5}$/u.test(batch.batchId))) {
        throw new Error("french-codex-custom-batch-id-invalid");
    }
    const ids = new Set();
    const bySize = Object.fromEntries(SIZE_ORDER.map((size) => [size, 0]));
    for (const batch of manifest.batches) {
        ...
        if (ids.has(batch.batchId) ||
            ...
            [...batch.proposerAViewHashes, ...batch.proposerBViewHashes].some((digest) => !SHA256_PATTERN.test(digest)) ||
        ...
        if (dirname(batch.inputs.proposerA.path) !== directory ||
    ...
    if (SIZE_ORDER.some((size) => bySize[size] !== manifest.counts.byMeaningSize[size]) ||
    ...
    if (sha256(sourceText) !== selection.sourceFileHash ||
    ...
    const viewsA = readViews(readFileSync(manifest.sourcePaths.proposerA, "utf8"), "proposerA");
    const viewsB = readViews(readFileSync(manifest.sourcePaths.proposerB, "utf8"), "proposerB");
    const packets = readPackets(readFileSync(manifest.sourcePaths.packets, "utf8"), new Set(keys));
    ...
    const batchPacketText = manifest.batches
        ...
        .join("");
    if (sha256(selectedText) !== manifest.selectedPackets.sha256 ||
        Buffer.byteLength(selectedText) !== manifest.selectedPackets.bytes ||
        selectedPackets.length !== manifest.selectedPackets.records ||
        selectedPackets.some((packet, index) => packet.entryKey !== expectedKeys[index]) ||
        ...
        selectedPackets.some((packet) => packets.get(packet.entryKey)?.packetHash !== packet.packetHash ||
    ...
    for (const batch of manifest.batches) {
        for (const [index, key] of batch.keys.entries()) {
            if (viewsA.get(key)?.viewHash !== batch.proposerAViewHashes[index] ||
                viewsB.get(key)?.viewHash !== batch.proposerBViewHashes[index] ||
                !packets.has(key)) {
                throw new Error(`french-codex-batch-source-view-stale:${key}`);
            }
            assertSelectedItemLineage(selection, viewsA.get(key), viewsB.get(key), packets.get(key), key);
        }
    }
...
export function assertFrenchCodexAnyBatchManifest(manifest, options = {}) {
    if (manifest.schemaVersion !== FRENCH_CODEX_BATCH_MANIFEST_SCHEMA_VERSION &&
        manifest.schemaVersion !== FRENCH_CODEX_PILOT_BATCH_MANIFEST_SCHEMA_VERSION) {
        throw new Error(`french-codex-batch-manifest-migration-required:${String(manifest.schemaVersion)}`);
    }
    if (manifest.schemaVersion === FRENCH_CODEX_BATCH_MANIFEST_SCHEMA_VERSION) {
        assertFrenchCodexBatchManifest(manifest, options);
...
function readPackets(text, selected) {
    const result = new Map();
    ...
    for (const [index, line] of text.split(/\r?\n/u).entries()) {
        ...
        if (issues.length)
            throw new Error(`french-codex-packet-invalid:${index + 1}:${issues.join(",")}`);
...
function writeFinalArtifact(physicalDir, finalDir, name, text) {
...
function writeSelectedPacketsArtifact(physicalRoot, finalRoot, packets, lineage) {
...
function readPacketSequence(path) {
    if (!existsSync(path)) {
    ...
    for (const [index, line] of readFileSync(path, "utf8")
        ...
        if (issues.length > 0 ||
            ...
            throw new Error(`french-codex-selected-packet-invalid:${index + 1}:${issues.join(",")}`);
        }
    ...
    if (result.length < 1)
        throw new Error("french-codex-selected-packets-empty");
    return result;
}
function assertArtifact(artifact, batchId) {
...
function installDirectoryAtomically(temp, output, backup) {
...
function assertOptions(options) {
    if (!SAFE_NAMESPACE_PATTERN.test(options.namespace) ||
    ...
    for (const size of SIZE_ORDER) {
        if (!Number.isInteger(options.maxItems[size]) ||
...
export function parseFrenchCodexBatchArgs(args) {
    ...
    const namespace = values.get("namespace") ?? (runKind === "full" ? "/fr-internal/full" : "");
    const expected = values.get("expected-entries");
    if (!expected)
        throw new Error("french-codex-expected-entries-required");
    ...
    return {
        runKind,
        namespace,
        selectionPath: resolve(values.get("selection") ?? `${DEFAULT_WORK}/shards.json`),
        proposerAPath: resolve(values.get("proposer-a") ?? `${DEFAULT_WORK}/proposer-a-input.jsonl`),
        proposerBPath: resolve(values.get("proposer-b") ?? `${DEFAULT_WORK}/proposer-b-input.jsonl`),
        ...
        ...(values.has("pilot-quality-gate")
            ...
            : {}),
        outputDir: resolve(values.get("output-dir") ?? `${DEFAULT_ROOT}/agent-batches/${runKind}`),
...
if (import.meta.url === pathToFileURL(resolve(process.argv[1] ?? "")).href) {
    try {
        ...
        process.stderr.write(`${basename(process.argv[1] ?? "buildLexiconV3FrenchCodexBatches")}: ${error instanceof Error ? error.message : String(error)}\n`);
        process.exitCode = 1;
    }
}
