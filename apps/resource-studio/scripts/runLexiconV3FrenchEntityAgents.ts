...
import {
  ...
  isFrenchEntityAgentMixedCanonicalView,
  ...
  type FrenchEntityAgentBatchRecord,
...
export const FRENCH_ENTITY_AGENT_RUN_SCHEMA_VERSION =
  "lexicon-v3-french-entity-agent-run@1" as const;
export const FRENCH_ENTITY_AGENT_RUNNER_POLICY_VERSION =
  "lexicon-v3-french-entity-agent-runner-policy@3" as const;
export const FRENCH_ENTITY_AGENT_ARBITER_INPUT_SCHEMA_VERSION =
  "lexicon-v3-french-entity-agent-arbiter-input@1" as const;
export const FRENCH_ENTITY_AGENT_AUDITOR_INPUT_SCHEMA_VERSION =
  "lexicon-v3-french-entity-agent-auditor-input@1" as const;
export const FRENCH_ENTITY_AGENT_PARSE_QUARANTINE_SCHEMA_VERSION =
  "lexicon-v3-french-entity-agent-parse-quarantine@1" as const;
...
export type FrenchEntityAgentRunStage =
  | "proposerA"
  | "proposerB"
  | "proposers"
  | "arbiter"
...
export interface FrenchEntityAgentRunCliOptions {
  manifest: string;
  resultsDir: string;
  releaseKey: string;
  stage: FrenchEntityAgentRunStage;
  concurrency: number;
  batchIds: string[];
...
export interface FrenchEntityAgentRun {
  schemaVersion: typeof FRENCH_ENTITY_AGENT_RUN_SCHEMA_VERSION;
  policyVersion: typeof FRENCH_ENTITY_AGENT_RUNNER_POLICY_VERSION;
  entityPolicyVersion: typeof FRENCH_ENTITY_AGENT_POLICY_VERSION;
...
export interface FrenchEntityAgentExecutionSummary {
  releaseKey: string;
  stage: FrenchEntityAgentRunStage;
  batches: number;
  runs: number;
...
export interface FrenchEntityAgentRoleExecutionInput {
  text: string;
  logicalHash: string;
  ...
  >;
  dependencies: FrenchEntityAgentRun[];
}
...
export function installFrenchEntityAgentSignalCleanup(
  supervisor: FrenchEntityAgentChildSupervisor
): () => void {
...
interface FailedProcessExecution {
  stdout: string;
  stderr: string;
  responseText: string;
  startedAt: string;
...
class FrenchEntityAgentProcessExecutionError extends Error {
  readonly processExecution: FailedProcessExecution;

  constructor(message: string, processExecution: FailedProcessExecution) {
    super(message);
...
export class FrenchEntityAgentRetryableAttemptError extends Error {
  ...
  constructor(error: unknown, quarantineDirectory: string | null) {
...
export async function runFrenchEntityAgentAttempts<T>(input: {
  maxAttempts: number;
  label: string;
  execute: (attempt: number) => Promise<T>;
}): Promise<T> {
  assertMaxAttempts(input.maxAttempts);
  let lastFailure: FrenchEntityAgentRetryableAttemptError | null = null;
  ...
  throw new Error(
    `french-entity-run-attempts-exhausted:${input.label}:${input.maxAttempts}:${errorMessage(lastFailure?.originalError)}`
  );
}

export function parseFrenchEntityAgentRunArgs(
  args: readonly string[]
): FrenchEntityAgentRunCliOptions {
  ...
  if (!releaseKey) throw new Error("french-entity-run-release-key-required");
  const stage = (values.get("stage") ?? "all") as FrenchEntityAgentRunStage;
  if (
    ![
      "proposerA",
      "proposerB",
  ...
  return {
    ...
    batchIds: (values.get("batch") ?? "")
      ...
      .filter(Boolean),
    offsetBatches: nonNegativeInteger(
      values.get("offset-batches"),
      0,
      "offset-batches"
    ),
...
export async function runFrenchEntityAgentsCli(
  options: FrenchEntityAgentRunCliOptions
): Promise<FrenchEntityAgentExecutionSummary> {
  ...
  if (
    ...
    throw new Error(
      `french-entity-run-release-or-plan-mismatch:${manifest.plan.releaseKey}:${options.releaseKey}`
    );
  }
  ...
  const releaseLock = acquireExclusiveRoleLock(
    join(options.resultsDir, "entity-agent-run.lock")
  );
  const childSupervisor = new FrenchEntityAgentChildSupervisor();
  const removeSignalHandlers =
    installFrenchEntityAgentSignalCleanup(childSupervisor);
  let executed = 0;
  let reused = 0;
  try {
    ...
    const runs = batches.flatMap((batch) =>
      roles
        ...
        .filter((run): run is FrenchEntityAgentRun => run !== null)
    );
...
async function runRole(input: {
  ...
  batch: FrenchEntityAgentBatchRecord;
  role: FrenchEntityAgentRole;
  childSupervisor: FrenchEntityAgentChildSupervisor;
}): Promise<{ run: FrenchEntityAgentRun; reused: boolean }> {
  const finalDir = roleDirectory(
  ...
  if (existsSync(finalDir)) {
    ...
    throw new Error(
      `french-entity-run-existing-result-missing:${input.role}:${input.batch.batchId}`
    );
  }
...
async function runRoleAttempt(input: {
  ...
  let processResult: ProcessExecution | null = null;
  ...
  try {
    ...
    writeFileSync(
      paths.outputSchema,
      `${canonicalFrenchEntityJson(executionInput.schema)}\n`,
      "utf8"
    );
    ...
    if (input.seenAttemptThreads.has(processResult.threadId)) {
      throw new Error(
        `french-entity-run-retry-thread-reused:${context.role}:${context.batch.batchId}`
      );
    }
    ...
    for (const dependency of executionInput.dependencies) {
      if (dependency.threadId === processResult.threadId) {
        throw new Error(
          `french-entity-run-thread-not-independent:${context.role}:${context.batch.batchId}`
        );
      }
    }
    ...
    const runContent = {
      ...
      releaseSnapshotFingerprint:
        context.plan.sourceLineage.releaseSnapshotFingerprint,
...
export function buildFrenchEntityAgentRoleExecutionInput(input: {
  ...
  batch: FrenchEntityAgentBatchRecord;
  role: FrenchEntityAgentRole;
}): FrenchEntityAgentRoleExecutionInput {
  if (input.role === "proposerA" || input.role === "proposerB") {
    ...
    const outputContracts = artifact.views.map((view) => {
      ...
      return {
        role: input.role as FrenchEntityAgentProposerRole,
        unitId: view.unitId,
    ...
    const schema = frenchEntityAgentProposalResponseSchema(
      input.role,
      outputContracts
    );
    return {
      ...
      unitInputHashes: new Map(
        artifact.views.map((view) => [view.unitId, view.viewHash])
      ),
      schema,
  ...
  const proposalA = loadArtifacts<FrenchEntityAgentProposal>(
    input.options.resultsDir,
    "proposerA",
    input.batch,
    input
  );
  const proposalB = loadArtifacts<FrenchEntityAgentProposal>(
    input.options.resultsDir,
    "proposerB",
    input.batch,
    input
  );
  const runA = requiredExistingRun(
    input.options.resultsDir,
    "proposerA",
    input.batch.batchId
  );
  const runB = requiredExistingRun(
    input.options.resultsDir,
    "proposerB",
    input.batch.batchId
  );
  if (runA.threadId === runB.threadId) {
    throw new Error(
      `french-entity-run-proposer-thread-collision:${input.batch.batchId}`
    );
  }
  ...
  const sourceArtifact = selectFrenchEntityAgentReviewSourceArtifact(
    loadProposerInputArtifact(input, "proposerA").artifact,
    loadProposerInputArtifact(input, "proposerB").artifact
  );
  const sourceViewByUnit = new Map(
    sourceArtifact.views.map((view) => [view.unitId, view])
  );
  if (input.role === "arbiter") {
    ...
    const unitInputHashes = new Map(
      records.map((record) => [record.unitId, record.viewHash])
    );
  ...
  const arbitrations = loadArtifacts<FrenchEntityAgentArbitration>(
    input.options.resultsDir,
    "arbiter",
    input.batch,
    input
  );
  const arbiterRun = requiredExistingRun(
    input.options.resultsDir,
    "arbiter",
    input.batch.batchId
  );
  ...
  const records = input.batch.unitIds.map((unitId) => {
    ...
    if (
      ...
      throw new Error(
        `french-entity-run-arbitration-selection-invalid:${unitId}`
      );
    }
 ...
 */
export function selectFrenchEntityAgentReviewSourceArtifact(
  proposerA: FrenchEntityAgentInputArtifact,
  proposerB: FrenchEntityAgentInputArtifact
): Omit<FrenchEntityAgentInputArtifact, "role" | "views"> & {
  role: "proposerB";
  views: FrenchEntityAgentProposerBView[];
} {
  if (
  ...
  return proposerB as Omit<FrenchEntityAgentInputArtifact, "role" | "views"> & {
    role: "proposerB";
    views: FrenchEntityAgentProposerBView[];
  };
}

function loadProposerInputArtifact(
  input: {
    options: FrenchEntityAgentRunCliOptions;
    plan: FrenchEntityCanonicalizationPlan;
    batch: FrenchEntityAgentBatchRecord;
...
): { artifact: FrenchEntityAgentInputArtifact; text: string } {
  const proof =
    role === "proposerA" ? input.batch.proposerA : input.batch.proposerB;
  const path = resolve(dirname(input.options.manifest), proof.relativePath);
  const text = readFileSync(path, "utf8");
  if (sha256(text) !== proof.sha256) {
    throw new Error(
      `french-entity-run-proposer-input-drift:${input.batch.batchId}:${role}`
    );
  }
...
function proposerPrompt(
  ...
  const hasUnboundNames = artifact.views.some(
    isFrenchEntityAgentUnboundNameView
  );
  const hasMixedCanonicalNames = artifact.views.some(
    isFrenchEntityAgentMixedCanonicalView
  );
  const hasUnboundAlternateNames = artifact.views.some(
...
async function executeCodex(input: {
  ...
  if (snapshotError !== null) {
    throw new FrenchEntityAgentProcessExecutionError(
  ...
  if (executionError !== null) {
    throw new FrenchEntityAgentProcessExecutionError(
      `french-entity-run-process-error:${input.role}:${errorMessage(executionError)}`,
      failedProcess
    );
  }
  if (timedOut || exitCode !== 0 || !existsSync(input.responsePath)) {
    throw new FrenchEntityAgentProcessExecutionError(
      timedOut
        ...
        : `french-entity-run-process-failed:${input.role}:${exitCode}`,
      failedProcess
    );
  }
  ...
  try {
    ...
    throw new FrenchEntityAgentProcessExecutionError(
      `french-entity-run-event-parse-failed:${input.role}:${errorMessage(error)}`,
      failedProcess
    );
  }
  return {
...
interface FrenchEntityAgentAttemptPaths {
  sealedInput: string;
  prompt: string;
  outputSchema: string;
...
export interface FrenchEntityAgentAttemptQuarantineRecord {
  schemaVersion: typeof FRENCH_ENTITY_AGENT_PARSE_QUARANTINE_SCHEMA_VERSION;
  policyVersion: typeof FRENCH_ENTITY_AGENT_RUNNER_POLICY_VERSION;
...
export function quarantineFrenchEntityAgentAttemptFailure(input: {
  ...
  const record: FrenchEntityAgentAttemptQuarantineRecord = {
    ...content,
  ...
  do {
    quarantineDirectory = join(
      quarantineRoot,
      `${failureHash}-${randomBytes(8).toString("hex")}`
    );
...
function materializeFailedProcessEvidence(
  paths: FrenchEntityAgentAttemptPaths,
  error: unknown
): void {
  if (error instanceof FrenchEntityAgentProcessExecutionError) {
    ...
    if (!existsSync(paths.structuredResponse)) {
      writeFileSync(
        ...
        error.processExecution.responseText,
        "utf8"
      );
    }
  }
...
function buildReceipts(input: {
  ...
  artifacts: Array<
    ...
    | FrenchEntityAgentAudit
  >;
  temporaryPaths: Record<string, string>;
  finalPaths: Record<string, string>;
}): Array<FrenchCodexExecutionReceipt<FrenchEntityAgentRole>> {
  const unitById = new Map(
    input.input.plan.reviewUnits.map((unit) => [unit.unitId, unit])
  );
  return input.artifacts.map((artifact) => {
    ...
    const receipt = finalizeFrenchCodexExecutionReceipt({
      ...
      inputHash: requiredMap(
        input.executionInput.unitInputHashes,
        artifact.unitId
      ),
...
function validateExistingResult(
  ...
  expectedExecution?: Pick<
    FrenchEntityAgentRoleExecutionInput,
    "text" | "logicalHash" | "prompt" | "schema"
  >
): FrenchEntityAgentRun {
  const run = requiredExistingRun(
    input.options.resultsDir,
    input.role,
    input.batch.batchId
  );
  if (
    ...
    throw new Error(
      `french-entity-run-existing-release-or-plan-mismatch:${input.role}:${input.batch.batchId}`
    );
  }
  if (expectedExecution) {
    ...
    for (const [file, expectedText] of Object.entries(expectedFiles)) {
      ...
      if (!existsSync(path) || readFileSync(path, "utf8") !== expectedText) {
        throw new Error(
          `french-entity-run-existing-input-drift:${input.role}:${input.batch.batchId}:${file}`
        );
      }
    }
    if (
      ...
      throw new Error(
        `french-entity-run-existing-source-binding:${input.role}:${input.batch.batchId}`
      );
    }
  }
...
function validateTemporaryResult(
  input: {
    batch: FrenchEntityAgentBatchRecord;
    role: FrenchEntityAgentRole;
  },
...
function validateResultFiles(
  ...
  for (const [key, file] of Object.entries(resultFileNames)) {
    ...
    if (
      ...
      throw new Error(
        `french-entity-run-result-drift:${role}:${run.batchId}:${key}`
      );
    }
  }
  ...
  if (
    artifacts.length !== expectedUnitIds.length ||
...
function assertReceiptFiles(
  ...
  for (const [key, declaredPath] of Object.entries(receipt.sourcePaths)) {
    ...
    if (hash !== receipt.sourceHashes[key]) {
      throw new Error(
        `french-entity-run-receipt-source-drift:${receipt.role}:${receipt.batchId}:${key}`
      );
    }
    if (key in run.sourceHashes && hash !== run.sourceHashes[key]) {
      throw new Error(
        `french-entity-run-receipt-source-run-drift:${receipt.role}:${receipt.batchId}:${key}`
      );
    }
  }
  for (const [key, declaredPath] of Object.entries(receipt.resultPaths)) {
    ...
    if (hash !== receipt.resultHashes[key] || hash !== run.resultHashes[key]) {
      throw new Error(
        `french-entity-run-receipt-result-drift:${receipt.role}:${receipt.batchId}:${key}`
      );
    }
  }
}

function existingReceiptPath(directory: string, declaredPath: string): string {
...
function loadArtifacts<T>(
  ...
  return readJsonl<T>(join(directory, "artifacts.jsonl"));
}

export function assertFrenchEntityAgentRun(run: FrenchEntityAgentRun): void {
  const { runHash, ...content } = run;
  const profile = FRENCH_INTERNAL_APPROVED_EXECUTION_PROFILE[run.role];
  if (
    run.schemaVersion !== FRENCH_ENTITY_AGENT_RUN_SCHEMA_VERSION ||
  ...
  for (const hash of [
    ...
    if (!SHA256_PATTERN.test(hash)) {
      throw new Error(
        `french-entity-run-hash-invalid:${run.role}:${run.batchId}`
      );
    }
  }
}
...
function readExistingRun(
  ...
  role: FrenchEntityAgentRole,
  batchId: string
): FrenchEntityAgentRun | null {
  const path = join(roleDirectory(resultsDir, role, batchId), "run.json");
  if (!existsSync(path)) return null;
  return JSON.parse(readFileSync(path, "utf8")) as FrenchEntityAgentRun;
}

function requiredExistingRun(
  resultsDir: string,
  role: FrenchEntityAgentRole,
  batchId: string
): FrenchEntityAgentRun {
...
function rolesForStage(
  stage: FrenchEntityAgentRunStage
): FrenchEntityAgentRole[] {
  if (stage === "all") return ["proposerA", "proposerB", "arbiter", "auditor"];
  if (stage === "proposers") return ["proposerA", "proposerB"];
  return [stage];
}

function assertDistinctThreads(runs: readonly FrenchEntityAgentRun[]): void {
...
function boundedPositiveInteger(
  ...
  if (parsed > maximum) {
    throw new Error(
      `french-entity-run-invalid-${label}:${parsed}:maximum-${maximum}`
    );
  }
  return parsed;
}
...
function isNonRetryableAgentAttemptError(error: unknown): boolean {
...
if (import.meta.url === invokedPath) {
  runFrenchEntityAgentsCli(
    parseFrenchEntityAgentRunArgs(process.argv.slice(2))
  ).catch((error: unknown) => {
    process.stderr.write(
      `${basename(process.argv[1] ?? "runLexiconV3FrenchEntityAgents")}: ${
