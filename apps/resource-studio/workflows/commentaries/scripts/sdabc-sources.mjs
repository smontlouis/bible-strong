...
export const selectBiblePdfs = (metadata) => {
  ...
  const missing = BIBLE_BOOKS.map((_, index) => index + 1).filter(
    (book) => !covered.has(book)
  );
  if (missing.length)
    throw new Error(
      `PDF SDABC manquants pour les livres : ${missing.join(", ")}`
    );
  return selected;
};
...
const markerRegex =
  /^[ \t]*(?:C[ \t]*HAPTER|PSALM)[ \t]+(\d+)\.?[ \t]*(?=$|INTRODUCTION)/gim;
...
export const splitBookChapters = ({ text, chapterCount }) => {
    ...
    return [
      {
        chapter: 1,
        text: normalized,
        introduction: normalized.slice(
          0,
          findSingleChapterCommentaryStart(normalized)
        )
      }
    ];
  }
  if (unique.length !== chapterCount) {
    throw new Error(
      `${chapterCount} chapitres attendus, ${unique.length} titres trouvés`
    );
  }
  return unique.map((marker, index) => ({
    ...
    text: normalized.slice(
      marker.end,
      unique[index + 1]?.index ?? normalized.length
    ),
...
const headingRegex =
  /^( {0,16})(\d{1,3}(?:\s*(?:[-–—,]|,\s*vs?\.)\s*\d{1,3})*)\.\s+(\S.*)$/gm;
...
const findSingleChapterCommentaryStart = (text) => {
  ...
  const startAt = outline
    ? outline.index + outline[0].length
...
const stripSupplement = (text) =>
  text.split(/^\s*ELLEN G\. WHITE COMMENTS\s*$/im, 1)[0];
...
const cleanParagraphs = (value) => {
  ...
  for (const rawLine of lines) {
    ...
    if (/^(?:\d+|Seventh-day Adventist Bible Commentary)$/i.test(line))
      continue;
    current.push(line);
  }
  flush();
...
export const parseChapterCommentary = ({
  text,
  book,
  chapter,
  maxVerse,
  ...
  const candidates = [...body.matchAll(headingRegex)]
    ...
    .filter(
      (item) =>
        item.anchor >= 1 && item.anchor <= maxVerse && item.endVerse <= maxVerse
    );
  ...
  for (const [candidateIndex, candidate] of candidates.entries()) {
    ...
    const laterCorrection = candidates
  ...
  if (!selected.length)
    throw new Error(`Aucun commentaire trouvé pour ${book}-${chapter}`);

  return selected.map((heading, index) => {
    const content = body
      .slice(heading.endIndex, selected[index + 1]?.index ?? body.length)
      .replace(/^\n/, "");
...
const withSourceHtml = (entry, html) => ({
  ...entry,
  source: { ...entry.source, html, sha256: sha256(html) }
});

/**
 * Genesis 39 contains an eight-point historical outline inside the note on verse 1.
 * Its points 2–8 resemble verse headings in the OCR. Rejoin that exact outline and
 * recover the real comments on verses 1, 2, 6 and 7 from the final captured block.
 */
export const repairGenesis39HistoricalOutline = (entries) => {
  const byPassage = new Map(entries.map((entry) => [entry.passage, entry]));
  const first = byPassage.get("1-39-1");
  const second = byPassage.get("1-39-2");
  const eighth = byPassage.get("1-39-8");
  const signature = second?.source.html.startsWith(
    "<p>The horse and chariot were introduced into Egypt by the Hyksos"
  );
  if (!signature) return entries;
  const outline = Array.from({ length: 7 }, (_, index) =>
    byPassage.get(`1-39-${index + 2}`)
  );
  if (
    !first ||
    outline.some((entry) => !entry) ||
    !eighth.source.html.includes("</p><p>An Egyptian.")
  ) {
    throw new Error("Structure connue de Genèse 39 incomplète");
  }
  const match = /^(<p>[\s\S]*?<\/p>)<p>(An Egyptian\.[\s\S]*)<\/p>$/u.exec(
    eighth.source.html
  );
  if (!match) throw new Error("Bloc final connu de Genèse 39 introuvable");
  const [verseOne, afterVerseTwo] = match[2].split(
    " 2. The Lord was with Joseph."
  );
  const [verseTwoTail, afterVerseSix] = (afterVerseTwo ?? "").split(
    " 6. A goodly person, and well favoured."
  );
  const [verseSixTail, verseSevenTail] = (afterVerseSix ?? "").split(
    " 7. His master’s wife."
  );
  if (
    [afterVerseTwo, afterVerseSix, verseSevenTail].some(
      (value) => value === undefined
    )
  ) {
    throw new Error("Commentaires réels de Genèse 39 introuvables");
  }
  const htmlByPassage = new Map([
    [
      "1-39-1",
      `${first.source.html}${outline.map((entry) => entry.source.html).join("")}${match[1]}<p>${verseOne}</p>`
    ],
    ["1-39-2", `<p>The Lord was with Joseph.${verseTwoTail}</p>`],
    ["1-39-6", `<p>A goodly person, and well favoured.${verseSixTail}</p>`],
    ["1-39-7", `<p>His master’s wife.${verseSevenTail}</p>`]
  ]);
  return entries
    .filter(
      (entry) =>
        !["1-39-3", "1-39-4", "1-39-5", "1-39-8"].includes(entry.passage)
    )
    .map((entry) =>
      htmlByPassage.has(entry.passage)
        ? withSourceHtml(entry, htmlByPassage.get(entry.passage))
        : entry
    );
};
...
export const splitCombinedJohn = (text) => {
  ...
  return {
    63: normalized.slice(0, third.index),
    64: normalized.slice(third.index)
  };
};
