...
const scriptureScopeParser = createBibleReferenceParser("en");
...
const detachString = (value) =>
  Buffer.from(String(value ?? ""), "utf8").toString("utf8");
...
export const normalizeEgwMarkup = (value) =>
  decodeEntities(String(value ?? ""))
    .replace(/<!--[\s\S]*?-->/g, "")
    .replace(
      /<(?:script|style|iframe|object|svg)\b[^>]*>[\s\S]*?<\/(?:script|style|iframe|object|svg)>/gi,
      ""
    )
    .replace(
      /<span\b[^>]*class="[^"]*refCode[^"]*"[^>]*>[\s\S]*?<\/span>/gi,
      ""
    )
    .replace(
      /<span\b[^>]*class="[^"]*egwlink_bible[^"]*"[^>]*>([\s\S]*?)<\/span>/gi,
      '<span class="ref">$1</span>'
    )
    .replace(
      /<span\b[^>]*class="[^"]*egwlink_book[^"]*"[^>]*>([\s\S]*?)<\/span>/gi,
      '<span class="source-ref">$1</span>'
    )
    .replace(/<a\b[^>]*>([\s\S]*?)<\/a>/gi, "$1")
    .replace(/<(?:b|strong)\b[^>]*>/gi, "<strong>")
    .replace(/<\/(?:b|strong)>/gi, "</strong>")
    .replace(/<(?:i|em)\b[^>]*>/gi, "<em>")
    .replace(/<\/(?:i|em)>/gi, "</em>")
    .replace(/<br\b[^>]*>/gi, "<br>")
    .replace(/<p\b[^>]*>/gi, "<p>")
    .replace(/<\/p>/gi, "</p>")
    .replace(/<span\b([^>]*)>/gi, (_, attributes) => {
      const safeClass = attributes
        .match(/\bclass="(ref|source-ref)"/i)?.[1]
        ?.toLowerCase();
...
const parseAnchor = ({ bookName, chapter, verseExpression }) => {
  ...
  if (!Number.isInteger(verse) || verse < 1)
    throw new Error(
      `Expression de verset EGW non reconnue : ${verseExpression}`
    );
  return {
    passage: `${bookNumber(normalizeBookName(bookName))}-${Number(chapter)}-${verse}`,
    passageEndVerse:
      Number(String(verseExpression).match(/\d+\s*[-–—]\s*(\d+)/)?.[1]) || verse
  };
};
...
export const parseCommentaryToc = ({ html, bookId, code }) => {
  ...
  for (const match of String(html).matchAll(
    /<a\b[^>]*href="\/read\/([\d.]+)"[^>]*class="([^"]*book-toc__link[^"]*)"[^>]*>([\s\S]*?)<\/a>/gi
  )) {
    ...
    if (currentBook && Number.isInteger(chapter))
      pages.push({ bookId, code, pageId, bookName: currentBook, chapter });
  }
...
export const parseScriptureIndexToc = (html) => {
  ...
  for (const match of String(html).matchAll(
    ...
    pages.push({
      bookId: EGW_SCRIPTURE_INDEX.bookId,
      code: EGW_SCRIPTURE_INDEX.code,
      pageId: match[1],
      bookName,
...
const paragraphMatches = (html) =>
  [...String(html).matchAll(/<p\b([^>]*)>([\s\S]*?)<\/p>/gi)].map((match) => ({
    attributes: match[1],
    inner: match[2],
...
const attribute = (attributes, name) =>
  String(attributes).match(new RegExp(`\\b${name}="([^"]*)"`, "i"))?.[1] ??
  null;

export const isChapterAssociationMarker = (value) =>
  /^This chapter is based on\b/i.test(plainText(value));
...
export const scriptureScopeCoversPassage = (osis, passage) => {
  ...
  return String(osis)
    ...
    .some((segment) => {
      ...
      return (
        compareLocation(current, normalizedStart) >= 0 &&
        compareLocation(current, end) <= 0
      );
    });
};
...
export const parseBookTocSectionPages = ({ html, sectionPageId }) => {
  const anchors = [...String(html).matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)]
    .map((match) => ({
      pageId:
        attribute(match[1], "href")?.match(/^\/read\/([\d.]+)$/)?.[1] ?? null,
      classes: attribute(match[1], "class") ?? "",
      end: match.index + match[0].length
    }))
    .filter(
      (anchor) =>
        anchor.pageId && anchor.classes.split(/\s+/).includes("book-toc__link")
    );
  const section = anchors.find((anchor) => anchor.pageId === sectionPageId);
  if (!section)
    throw new Error(
      `Section EGW absente de la table des matières : ${sectionPageId}`
    );
  if (!section.classes.split(/\s+/).includes("js-toc_e"))
    return [sectionPageId];
  const sublist = String(html)
    .slice(section.end)
    .match(
      /^\s*<ul\b[^>]*class="[^"]*book-toc__sublist[^"]*"[^>]*>([\s\S]*?)<\/ul>/i
    )?.[1];
  if (!sublist) return [sectionPageId];
  const childPageIds = [...sublist.matchAll(/<a\b([^>]*)>/gi)]
    .map(
      (match) => attribute(match[1], "href")?.match(/^\/read\/([\d.]+)$/)?.[1]
    )
    .filter(Boolean);
...
const breadcrumb = (html, hrefPattern) => {
  ...
  for (const match of String(html).matchAll(/<a\b([^>]*)>([\s\S]*?)<\/a>/gi)) {
    ...
    if (
      href &&
      classes.split(/\s+/).includes("breadcrumbs-item") &&
...
export const parseIndexedParagraphContext = ({ html, paragraphId }) => {
  ...
  const sectionTitle =
    plainText(
      String(html).match(
        /<h1\b[^>]*class="[^"]*breadcrumbs-header-title[^"]*"[^>]*>([\s\S]*?)<\/h1>/i
      )?.[1]
    ) || section?.label;

  if (!bookTitle || !sectionTitle)
    throw new Error("Métadonnées de contexte EGW absentes");
...
const indexedParagraphDocument = ({
  paragraph,
  context,
  chapterAssociation = null
}) => {
  const sourceReference = plainText(
    paragraph.inner.match(
      /<span\b[^>]*class="[^"]*refCode[^"]*"[^>]*>([\s\S]*?)<\/span>/i
    )?.[1]
  );
  const body = detachString(normalizeEgwMarkup(paragraph.inner));
  if (!plainText(body))
    throw new Error(`Paragraphe EGW vide : ${paragraph.id}`);
...
export const parseIndexedParagraphPage = ({ html, targetParagraphIds }) => {
  const targets =
    targetParagraphIds instanceof Set
      ? targetParagraphIds
      : new Set(targetParagraphIds);
  const firstTarget = targets.values().next().value ?? "unknown";
  const pageContext = parseIndexedParagraphContext({
    html,
    paragraphId: firstTarget
  });
  const paragraphs = [];
  for (const match of String(html).matchAll(/<p\b([^>]*)>([\s\S]*?)<\/p>/gi)) {
    ...
    paragraphs.push(
      indexedParagraphDocument({
        paragraph: { id, inner: match[2] },
        context: pageContext
      })
    );
  }
...
export const isIndexedSectionAnchor = ({ html, targetParagraphId }) => {
  const paragraphs = paragraphMatches(html).filter(
    (paragraph) =>
      paragraph.id && paragraph.classes.split(/\s+/).includes("para")
  );
...
export const parseIndexedHeadingSectionPage = ({
  html,
  context,
  anchorParagraphId
}) =>
  paragraphMatches(html)
    ...
    .filter(
      (paragraph) =>
        paragraph.id !== anchorParagraphId &&
        !isChapterAssociationMarker(paragraph.inner)
    )
    .map((paragraph) => indexedParagraphDocument({ paragraph, context }));

export const parseIndexedSectionPage = ({
  html,
  context,
  markerParagraphId,
  markerText
  ...
  paragraphMatches(html)
    ...
    .filter(
      (paragraph) =>
        paragraph.id !== markerParagraphId &&
        !isChapterAssociationMarker(paragraph.inner)
    )
    .map((paragraph) =>
      indexedParagraphDocument({
...
export const parseCommentaryPage = ({ html, page }) => {
  ...
  for (const paragraph of paragraphMatches(html)) {
    ...
    const primary = paragraph.inner.match(
      /<span\b[^>]*class="[^"]*egwlink_bible[^"]*"[^>]*title="([^"]+)"[^>]*>/i
    );
    const reference = decodeEntities(
      primary?.[1] ?? `${page.bookName} ${page.chapter}:1`
    );
    const parsedReference = reference.match(
      /^(.+?)\s+(\d+):(\d+)(?:\s*[-–—]\s*(\d+))?$/
    );
    const anchor = parsedReference
      ? parseAnchor({
          bookName: parsedReference[1],
          chapter: parsedReference[2],
          verseExpression: parsedReference[4]
            ? `${parsedReference[3]}-${parsedReference[4]}`
            : parsedReference[3]
        })
      : parseAnchor({
    ...
    const sourceLinks = [
      ...paragraph.inner.matchAll(
        /<span\b[^>]*class="[^"]*egwlink_book[^"]*"[^>]*title="([^"]+)"[^>]*data-link="([^"]+)"[^>]*>/gi
      )
    ].map((match) => ({
      label: decodeEntities(match[1]),
      paragraphId: match[2],
      url: `https://text.egwwritings.org/read/${match[2]}`
    }));
    entries.push({
      schemaVersion: 1,
      ...
      resource: {
        id: `egw-${page.code.toLowerCase()}`,
        name: `EGW SDA Bible Commentary ${page.code}`,
        author: "Ellen G. White",
        sourceLanguage: "en",
        license: "CustomPermission"
      },
      source: {
        language: "en",
        html: htmlBody,
        sha256: sha256(htmlBody),
        provenance: `EGW Writings · ${page.code} · ${paragraph.id}`,
        url: `https://text.egwwritings.org/read/${paragraph.id}`
      },
      translation: null,
...
export const parseScriptureIndexPage = ({ html, page }) => {
  ...
  for (const paragraph of paragraphs) {
    ...
    if (!verseExpression || !paragraph.classes.split(/\s+/).includes("para"))
      continue;
    const citations = [
      ...paragraph.inner.matchAll(
        /<span\b[^>]*class="[^"]*egwlink_book[^"]*"[^>]*title="([^"]+)"[^>]*data-link="([^"]+)"[^>]*>/gi
    ...
    const uniqueCitations = [
      ...new Map(
        citations.map((citation) => [citation.paragraphId, citation])
      ).values()
    ];
...
export const mergeEgwLayers = ({ commentary, scriptureIndex }) => {
  ...
  for (const entry of commentary)
    ensure(entry.passage).curatedCommentary.push(entry);
  for (const entry of scriptureIndex)
    ensure(entry.passage).scriptureIndex.push(entry);
