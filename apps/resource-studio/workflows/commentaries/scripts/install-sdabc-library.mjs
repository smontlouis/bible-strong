...
const sdabcRoot = path.resolve(process.argv[2] ?? path.join(prototypeRoot, '.local/sdabc-export'))
const egwRoot = path.resolve(process.argv[3] ?? path.join(prototypeRoot, '.local/egw-export'))
const libraryRoot = path.resolve(process.argv[4] ?? path.join(prototypeRoot, '.local/library'))
...
const verifyArtifact = async (root, manifest, key) => {
  ...
  if (sha256(raw) !== artifact.sha256) throw new Error(`Hash invalide : ${artifact.path}`)
...
if (sdabcManifest.authorization?.status !== 'confirmed-by-project-owner') throw new Error('Autorisation SDABC absente')
if (egwManifest.authorization?.status !== 'confirmed-by-project-owner') throw new Error('Autorisation EGW absente')
...
if (general.length !== sdabcManifest.counts.entries) throw new Error('Compte SDABC incohérent')
if (curated.length !== egwManifest.counts.commentaryEntries) throw new Error('Compte EGW 1–7BC incohérent')
if (scriptureIndex.length !== egwManifest.counts.scriptureIndexEntries) throw new Error('Compte ECSI incohérent')
...
const entries = [
  ...general.map(entry => ({ ...entry, resource, layer: 'general-commentary' })),
  ...curated.map(entry => ({
    ...entry,
    resource,
...
const chapters = new Map(index.chapters.map(chapter => {
  ...
  return [`${chapter.book}-${chapter.chapter}`, { ...chapter, resources, passages: new Set(chapter.passages) }]
}))
const bookNames = new Map(index.chapters.map(chapter => [chapter.book, chapter.bookName]))
...
const layerOrder = { 'general-commentary': 0, 'book-introduction': 0, 'egw-supplement': 1, 'egw-scripture-index': 2 }
for (const [key, chapterEntries] of groups) {
  chapterEntries.sort((left, right) =>
    ...
    (layerOrder[left.layer ?? left.editorialKind] ?? 9) - (layerOrder[right.layer ?? right.editorialKind] ?? 9) ||
    left.id.localeCompare(right.id, 'en', { numeric: true })
  )
  const [book, chapter] = key.split('-').map(Number)
  const relativePath = `chunks/${book}/${chapter}/sdabc.json`
  const payload = JSON.stringify({ schemaVersion: 1, resourceId: 'sdabc', entries: chapterEntries })
  ...
  record.resources.sdabc = { path: relativePath, count: chapterEntries.length, sha256: sha256(payload) }
...
index.chapters = [...chapters.values()]
  ...
  .map(chapter => ({
    ...chapter,
    passages: [...chapter.passages].sort((left, right) => left.localeCompare(right, 'en', { numeric: true })),
  }))
await writeFile(path.join(libraryRoot, 'index.json'), JSON.stringify(index, null, 2) + '\n')
const normalization = await normalizeLibraryScopes(libraryRoot)
process.stdout.write(JSON.stringify({ resource: 'sdabc', entries: entries.length, chapters: groups.size, layers: index.resources.sdabc.layers, normalization }, null, 2) + '\n')
