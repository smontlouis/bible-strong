...
const chapterKey = (passage) =>
  String(passage).split("-").slice(0, 2).join("-");
...
const coveredPassages = (entry, chaptersByKey) => {
  ...
  const records = [...chaptersByKey.values()]
    ...
    .filter((chapter) => {
      if (chapter.book === start.book && chapter.chapter < start.chapter)
        return false;
      if (chapter.book === end.book && chapter.chapter > end.chapter)
        return false;
      return true;
    })
    .sort(
      (left, right) => left.book - right.book || left.chapter - right.chapter
    );
  const result = [];
  for (const chapter of records) {
    const existingMax = Math.max(
      0,
      ...[...chapter.passages].map(
        (passage) => parsePassage(passage)?.verse ?? 0
      )
    );
    const firstVerse =
      chapter.book === start.book && chapter.chapter === start.chapter
        ? start.verse
        : 1;
    const lastVerse =
      chapter.book === end.book && chapter.chapter === end.chapter
        ? end.verse
        : existingMax;
...
const anchorTranslationCounts = (entry) => {
  if (!entry.sourceAnchors?.length)
    return { anchors: 1, translated: entry.translation ? 1 : 0 };
  ...
  for (const anchor of entry.sourceAnchors.slice(1))
    if (variants.get(anchor.id)) translated += 1;
  return { anchors: entry.sourceAnchors.length, translated };
};

export const normalizeLibraryScopes = async (
  ...
  for (const chapter of chapters.values()) {
    for (const [resourceId, descriptor] of Object.entries(chapter.resources)) {
      ...
      const entries = normalizeRepeatedSourceEntries(payload.entries);
  ...
  for (const chunk of normalizedChunks) {
    ...
    resource.translatedCount += chunk.entries.filter(
      (entry) => entry.translation
    ).length;
    resource.missingCount += chunk.entries.filter(
      (entry) => !entry.translation
    ).length;
    ...
    for (const entry of chunk.entries) {
      ...
      for (const passage of passages) {
        ...
        if (target !== chunk.chapter) {
          ...
          if (
            !target.coverageChunks.some(
              (candidate) => `${candidate.resourceId}:${candidate.path}` === key
            )
          ) {
            target.coverageChunks.push({
              resourceId: chunk.resourceId,
              path: chunk.descriptor.path
            });
          }
        }
      }
  ...
  index.chapters = [...chapters.values()]
    ...
    .map((chapter) => ({
      ...
      coverageChunks: chapter.coverageChunks.sort((left, right) =>
  ...
  return {
    ...
    units: [...counts.values()].reduce(
      (total, resource) => total + resource.entryCount,
      0
    ),
    sourceAnchors: [...counts.values()].reduce(
      (total, resource) => total + resource.sourceAnchorCount,
      0
    ),
    coverageChunks: index.chapters.reduce(
      (total, chapter) => total + chapter.coverageChunks.length,
      0
    ),
    links
  };
  ...
  normalizeLibraryScopes(libraryRoot)
    .then((result) =>
      process.stdout.write(
        `${JSON.stringify({ libraryRoot, ...result }, null, 2)}\n`
      )
    )
    .catch((error) => {
