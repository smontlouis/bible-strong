...
export const COMMENTARY_LINK_NORMALIZATION_REVISION = 'commentary-links-osis-v1'
export const COMMENTARY_BCV_PARSER_VERSION = '3.2.0-bible-strong.2'
...
const JUNK_DOMAINS = new Set(['28-29.su', 'biblehub.com', 'ref.ly', 'www.compassion.com'])
const rejectedBcvLabel = label => {
  const trimmed = label.trim()
  return /^le\s+(?:chapitre|texte|verset)\b/iu.test(trimmed) ||
...
const decodeTextWithOffsets = raw => {
  ...
  for (let index = 0; index < raw.length;) {
    const entity = raw[index] === '&' ? /^&(?:#\d+|#x[\da-f]+|[a-z]+);/iu.exec(raw.slice(index)) : null
...
const stripMarkup = html => decodeTextWithOffsets(String(html).replace(/<[^>]*>/gu, ' ')).text
  .replace(/\s+/gu, ' ')
  .trim()
...
const passageContext = passage => {
  const [book, chapter, verse] = String(passage ?? '').split('-').map(Number)
...
const validOsis = osis => /^(?:[1-4]?[A-Za-z]+\.\d+(?:\.\d+)?)(?:-(?:[1-4]?[A-Za-z]+\.)?\d+(?:\.\d+)?)?(?:,(?:[1-4]?[A-Za-z]+\.\d+(?:\.\d+)?)(?:-(?:[1-4]?[A-Za-z]+\.)?\d+(?:\.\d+)?)?)*$/u.test(osis)
...
const candidatesFor = (text, preferredLanguage, context = '') => {
  ...
  for (const [priority, language] of languages.entries()) {
    const parsed = context
      ? parsers[language].parse_with_context(text, context).osis_and_indices()
      : parsers[language].parse(text).osis_and_indices()
    for (const candidate of parsed) {
      const [start, end] = candidate.indices
      if (!validOsis(candidate.osis) || start === undefined || end === undefined || end <= start) continue
  ...
  for (const candidate of candidates) {
    if (!selected.some(current => candidate.start < current.end && current.start < candidate.end)) selected.push(candidate)
  }
...
const exactOsis = (text, language, context = '') => {
  ...
  for (const candidate of candidatesFor(trimmed, language, context)) {
    if (/^\s*$/u.test(trimmed.slice(0, candidate.start)) && REFERENCE_TAIL_PATTERN.test(trimmed.slice(candidate.end))) {
...
const providerOsis = (resourceId, href) => {
  ...
  if ((resourceId === 'acbc' || resourceId === 'barnes') && /^\/[1-4]?[A-Za-z]+_\d+\.\d+$/u.test(normalizedHref)) {
    return parseHrefWithBcv(normalizedHref.replace('_', '.'))
  }
  if (resourceId === 'aquifer-fr' && /^https:\/\/ref\.ly\//iu.test(normalizedHref)) {
    return parseHrefWithBcv(new URL(normalizedHref).pathname)
  }
  if (resourceId === 'mhy-fr' && /^[1-4]?[A-Za-z]{2,4}\d+[.:]\d+/u.test(normalizedHref)) {
    return parseHrefWithBcv(normalizedHref)
  }
  if (resourceId === 'calvin' && normalizedHref.startsWith('?scrBook=')) {
    const parameters = new URLSearchParams(normalizedHref.slice(1).split('#')[0])
    const book = parameters.get('scrBook')
    const chapter = parameters.get('scrCh')
    const verse = parameters.get('scrV')
    return book && chapter ? parseHrefWithBcv(`${book}.${chapter}${verse ? `.${verse}` : ''}`) : ''
  }
  return ''
}
...
const transformAnchors = (html, state) => {
  ...
  for (const match of html.matchAll(HTML_TOKEN_PATTERN)) {
    ...
    if (osis) {
      stack.at(-1).html += state.registerReference(frame.html, label, osis, source, 'exact')
...
const transformSemanticReferences = (html, state) => html
  .replace(/<span\b[^>]*class=["']ref["'][^>]*>([\s\S]*?)<\/span>/giu, (match, inner) => {
    const label = stripMarkup(inner)
    const declaredOsis = readAttribute(match, 'data-osis').replace(/^Bible:/iu, '')
    const osis = (declaredOsis && parseHrefWithBcv(declaredOsis)) ||
    ...
    return state.registerReference(inner, label, osis, declaredOsis ? 'osis-attribute' : 'source-marker', 'exact')
  })
  .replace(/<scripRef\b[^>]*>([\s\S]*?)<\/scripRef>/giu, (match, inner) => {
    const label = stripMarkup(inner)
    const osis = exactOsis(label, state.language) || exactOsis(label, state.language, state.context)
    if (!osis) return inner
    state.stats.semanticReferences += 1
    return state.registerReference(inner, label, osis, 'source-marker', 'exact')
  })
...
const annotateText = (raw, state) => {
  ...
  for (const candidate of candidates) {
    ...
    result += state.registerReference(inner, label, candidate.osis, 'bcv-text', 'high')
...
const annotateResidualText = (html, state) => {
  ...
  for (const match of html.matchAll(HTML_TOKEN_PATTERN)) {
    ...
    const opensSkipped = /^<(?:script|style|code|pre)\b/iu.test(token) ||
      (/^<span\b/iu.test(token) && /\bclass=["'](?:bible-ref|source-ref)["']/iu.test(token))
    const opensElement = /^<[a-z][^>]*>$/iu.test(token) && !/^<(?:br|hr|img|input|meta|link)\b/iu.test(token) && !/\/\s*>$/u.test(token)
...
export const normalizeCommentaryContent = ({ html, resourceId, language, passage, references = null, externalSources = null }) => {
  if (!html) return { html: html ?? '', references: [], externalSources: [], stats: emptyStats() }
  if (Array.isArray(references) && /\bdata-reference-id=/u.test(html)) {
    const rejectedIds = new Set(references
      .filter(reference => reference.source === 'bcv-text' && rejectedBcvLabel(reference.label))
      .map(reference => reference.id))
    const retainedReferences = references.filter(reference => !rejectedIds.has(reference.id))
    let retainedHtml = html
    for (const id of rejectedIds) {
      retainedHtml = retainedHtml.replace(
        new RegExp(`<span\\s+class="bible-ref"\\s+data-reference-id="${id}">([\\s\\S]*?)<\\/span>`, 'gu'),
        '$1'
      )
    }
    const curatedExternalSources = (externalSources ?? [])
      ...
      .filter(Boolean)
    const stats = { ...emptyStats(), references: retainedReferences.length, externalSources: curatedExternalSources.length }
    stats.providerReferences = retainedReferences.filter(reference => reference.source === 'provider-href').length
    stats.semanticReferences = retainedReferences.filter(reference => reference.source === 'osis-attribute' || reference.source === 'source-marker').length
    stats.bcvReferences = retainedReferences.filter(reference => reference.source === 'bcv-text').length
    let nextReferenceNumber = Math.max(0, ...retainedReferences.map(reference => Number(reference.id.slice(1)) || 0)) + 1
    const state = {
      resourceId,
      ...
      registerReference: (inner, label, osis, source, confidence) => {
        ...
        retainedReferences.push({ id, kind: 'bible', osis, label, source, confidence })
        stats.references += 1
        return `<span class="bible-ref" data-reference-id="${id}">${inner}</span>`
      },
    }
    const normalizedHtml = rejectedIds.size ? annotateResidualText(retainedHtml, state) : retainedHtml
    return {
  ...
  const state = {
    ...
    registerReference: (inner, label, osis, source, confidence) => {
      ...
      normalizedReferences.push({ id, kind: 'bible', osis, label, source, confidence })
      stats.references += 1
      return `<span class="bible-ref" data-reference-id="${id}">${inner}</span>`
    },
  }
  ...
  const deduplicatedSources = [...new Map(normalizedExternalSources.map(source => [`${source.url}\u0000${source.label}`, source])).values()]
  stats.externalSources = deduplicatedSources.length
  return { html: normalizedHtml, references: normalizedReferences, externalSources: deduplicatedSources, stats }
}
...
export const normalizeEntryLinks = entry => {
  ...
  const normalizeContent = content => {
    ...
    if (normalized.externalSources.length) result.externalSources = normalized.externalSources
    return result
  }
  const normalized = {
    ...entry,
  ...
  if (entry.translationVariants) {
    normalized.translationVariants = entry.translationVariants.map(variant => ({
      ...variant,
...
export const isValidCommentaryReference = reference =>
  ...
  ['provider-href', 'anchor-label', 'osis-attribute', 'source-marker', 'bcv-text'].includes(reference.source) &&
