...
export type SupplementaryCanonicalPublication =
  CanonicalCommentaryPublication | CanonicalCrossReferencePublication;

type SupplementaryCanonicalContent =
  | Omit<
      CanonicalCommentaryPublication,
      "revision" | "sourceVersion" | "sourceSha256"
    >
  | Omit<
...
type PartialSupplementaryCanonicalPublication = {
  format?:
    | CanonicalCommentaryPublication["format"]
    | CanonicalCrossReferencePublication["format"];
...
const readSqlite = async (
  ...
  if (resourceId === "MHY") {
    ...
    for (const row of rows) {
      ...
      try {
        ...
        throw new Error("supplementary-publication-commentary-json-invalid", {
          cause
        });
      }
      if (!isRecord(decoded))
    ...
    verses.sort((left, right) =>
      parseVerse(left.verseKey)
        .join("-")
        .localeCompare(parseVerse(right.verseKey).join("-"), undefined, {
          numeric: true
        })
    );
    return {
  ...
  const anchors = rows.flatMap((row) => {
    if (!verseKey(row.id))
      throw new Error("supplementary-publication-cross-reference-key-invalid");
    let decoded: unknown;
    try {
      decoded = JSON.parse(row.commentaires);
    } catch (cause) {
      throw new Error(
        "supplementary-publication-cross-reference-json-invalid",
        { cause }
      );
    }
    if (
    ...
    const references = (decoded as string[])
  ...
  return {
    ...
    counts: {
      ...
      references: anchors.reduce(
        (total, anchor) => total + anchor.references.length,
        0
      )
    }
  };
...
const validateMetadata = (metadata: SupplementaryPublicationMetadata) => {
  for (const [label, value] of [
    ...
    if (!value.trim())
      throw new Error(`supplementary-publication-${label}-missing`);
  }
  if (
...
const writeMetadata = async (
  sqlitePath: string,
  canonical: SupplementaryCanonicalPublication
) => {
...
export async function buildSupplementaryResourcePublication(
  ...
  if (!existsSync(sourceSqlitePath))
    throw new Error("supplementary-publication-source-missing");
  if (existsSync(outputDir))
    throw new Error("supplementary-publication-output-exists");
  ...
  return commitResourcePublicationBundle({
    ...
    validate: validateSupplementaryResourcePublication
  });
}
...
export async function buildAllSupplementaryResourcePublications(options: {
  ...
  const staging = await mkdtemp(
    path.join(tmpdir(), "supplementary-publications-")
  );
  try {
...
const decodeCanonical = (value: unknown): SupplementaryCanonicalPublication => {
  if (
    !isRecord(value) ||
    (value.format !== "bible-strong-canonical-commentary" &&
      value.format !== "bible-strong-canonical-cross-references")
  ) {
  ...
  if (candidate.resourceId === "MHY") {
    if (!Array.isArray(candidate.verses) || candidate.verses.length === 0)
      throw new Error("supplementary-publication-canonical-invalid");
    const keys = new Set<string>();
    for (const verse of candidate.verses) {
      if (
        !isRecord(verse) ||
        !isNonEmptyString(verse.verseKey) ||
        !verseKey(verse.verseKey) ||
        typeof verse.content !== "string" ||
        !verse.content.trim() ||
        keys.has(verse.verseKey)
      )
        throw new Error("supplementary-publication-canonical-verse-invalid");
    ...
    for (const anchor of candidate.verseAnchors) {
      if (
        ...
        anchor.references.some(
          (reference) => typeof reference !== "string" || !reference.trim()
        ) ||
        keys.has(anchor.verseKey)
      )
        throw new Error("supplementary-publication-canonical-anchor-invalid");
...
const decodeManifest = (
  value: unknown
): SupplementaryResourcePublicationManifest => {
  ...
  if (
    ...
    candidate.identity.language !== "fr" ||
    (candidate.identity.resourceId !== "MHY" &&
      candidate.identity.resourceId !== "TRESOR") ||
    (candidate.identity.kind !== "commentary" &&
      candidate.identity.kind !== "cross-references") ||
    envelope.canonical.schemaVersion !== 1 ||
    (candidate.identity.resourceId === "MHY" &&
      (candidate.identity.kind !== "commentary" ||
        envelope.offlineArtifact.entry !== "commentaires-mhy.sqlite")) ||
    (candidate.identity.resourceId === "TRESOR" &&
      (candidate.identity.kind !== "cross-references" ||
        envelope.offlineArtifact.entry !== "commentaires-tresor.sqlite")) ||
    !isRecord(candidate.counts) ||
    Object.values(candidate.counts).some(
...
export async function validateSupplementaryResourcePublication(
  bundleDir: string
): Promise<SupplementaryResourcePublicationManifest> {
  ...
  if (
    ...
    canonical.revision !== deriveRevision(canonical) ||
    canonical.sourceVersion !== manifest.provenance.sourceVersion ||
    canonical.sourceSha256 !== manifest.provenance.sourceSha256
  )
    throw new Error("supplementary-publication-declaration-mismatch");
  const extracted = await mkdtemp(
    path.join(tmpdir(), "supplementary-publication-validate-")
  );
  try {
    ...
    if (
      ...
      throw new Error("supplementary-publication-offline-metadata-mismatch");
  } finally {
...
const parseCliArgs = (args: readonly string[]) => {
  ...
  for (let index = 0; index < args.length; index += 2) {
    ...
    if (
      ...
      throw new Error(
        `supplementary-publication-cli-argument-invalid:${key ?? ""}`
      );
    result[key] = value;
  }
  return result;
...
const main = async () => {
  ...
  if (command === "validate") {
    console.log(
      JSON.stringify(
        await validateSupplementaryResourcePublication(args["--bundle"]),
        null,
        2
      )
    );
  ...
  if (command === "build") {
    ...
    ) as SupplementaryPublicationMetadata;
    const result = await buildSupplementaryResourcePublication({
      ...metadata,
    ...
    console.log(
      JSON.stringify(
        {
          resourceId: result.manifest.identity.resourceId,
  ...
  const mhyMetadata = JSON.parse(
    await readFile(path.resolve(args["--mhy-metadata"]), "utf8")
  ) as Omit<SupplementaryPublicationMetadata, "resourceId">;
  const tresorMetadata = JSON.parse(
    await readFile(path.resolve(args["--tresor-metadata"]), "utf8")
  ) as Omit<SupplementaryPublicationMetadata, "resourceId">;
  const manifests = await buildAllSupplementaryResourcePublications({
    mhySqlitePath: args["--mhy-sqlite"],
    tresorSqlitePath: args["--tresor-sqlite"],
    outputDir: args["--output-dir"],
    mhyMetadata,
    tresorMetadata
  });
