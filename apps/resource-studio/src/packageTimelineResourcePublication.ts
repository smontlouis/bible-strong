...
const toArray = <T>(value: unknown, map: (item: unknown) => T): T[] =>
...
const normalizeEvent = (value: unknown): TimelineEvent => {
  ...
  const related = toArray(value.related, (item) => {
    if (
      ...
      throw new Error("timeline-publication-relation-invalid");
  ...
  const images = toArray(value.images, (item) => {
    if (
      ...
      throw new Error("timeline-publication-image-invalid");
  ...
  const videos = toArray(value.videos, (item) => {
    if (
      ...
      throw new Error("timeline-publication-video-invalid");
    return {
      title: item.title,
  ...
  if (
    ...
    throw new Error("timeline-publication-event-invalid");
  return {
    id: value.id,
    slug: value.slug,
    title: value.title,
    ...
    scriptures: toArray(value.scriptures, (item) => {
      if (typeof item !== "string")
        throw new Error("timeline-publication-scripture-invalid");
      return item;
    })
  };
};
...
const readSource = async (sourcePath: string, language: Language) => {
  ...
  if (!Array.isArray(source) || source.length === 0)
    throw new Error("timeline-publication-source-empty");
  ...
  return {
    language,
    events: [...bySlug.values()].sort((a, b) => a.slug.localeCompare(b.slug))
  };
};
...
const decodeCanonical = (value: unknown): TimelineCanonical => {
  if (
    !isRecord(value) ||
    value.format !== "bible-strong-canonical-timeline" ||
    value.schemaVersion !== 1 ||
    value.resourceId !== "TIMELINE" ||
    !isLanguage(value.language) ||
    !isNonEmptyString(value.revision) ||
    !isNonEmptyString(value.sourceVersion) ||
    !/^[a-f0-9]{64}$/u.test(String(value.sourceSha256)) ||
    !Array.isArray(value.events) ||
    value.events.length === 0
  )
    throw new Error("timeline-publication-canonical-invalid");
  return value as unknown as TimelineCanonical;
};

const deriveRevision = (
  canonical: Pick<TimelineCanonical, "language" | "events">
) =>
  `timeline-${canonical.language}-${sha256(JSON.stringify(normalize(canonical.events))).slice(0, 20)}`;
const counts = (events: TimelineEvent[]) => ({
  events: events.length,
  relations: events.reduce((total, event) => total + event.related.length, 0),
  scriptures: events.reduce(
    (total, event) => total + event.scriptures.length,
    0
  )
});
...
const validateMetadata = (metadata: TimelineMetadata) => {
  if (
    !isLanguage(metadata.language) ||
    !metadata.sourceVersion.trim() ||
    !metadata.rights.holder.trim() ||
    !metadata.rights.termsReference.trim() ||
    !metadata.rights.attribution.trim() ||
    (!metadata.deliveryCapabilities.onlineAccess &&
      !metadata.deliveryCapabilities.offlineDownload)
  )
    throw new Error("timeline-publication-metadata-invalid");
  if (
    ...
    throw new Error("timeline-publication-rights-mismatch");
};
...
export const buildTimelineResourcePublication = async (
  ...
  if (!existsSync(sourcePath) || existsSync(outputDir))
    throw new Error("timeline-publication-path-invalid");
  ...
  const baseCanonical = {
    format: "bible-strong-canonical-timeline" as const,
    schemaVersion: 1 as const,
    resourceId: "TIMELINE" as const,
  ...
  return commitResourcePublicationBundle({
    ...
    build: async (staging) => {
      ...
      const canonicalPath = path.join(
        canonicalDir,
        `timeline-${options.language}.json`
      );
      ...
      const archivePath = path.join(
        offlineDir,
        "bible-timeline-events.json.zip"
      );
      await zipSingle(contentPath, archivePath);
      const [canonicalStat, archiveStat, contentStat] = await Promise.all([
        stat(canonicalPath),
        stat(archivePath),
        stat(contentPath)
      ]);
      const manifest: TimelineManifest = {
        ...
        revision,
        canonical: {
          path: `canonical/timeline-${options.language}.json`,
          mediaType: "application/json",
          schemaVersion: 1,
        ...
        offlineArtifact: {
          path: "offline/bible-timeline-events.json.zip",
          mediaType: "application/zip",
          entry: "bible-timeline-events.json",
          sha256: await sha256ResourcePublicationFile(archivePath),
          bytes: archiveStat.size,
          contentSha256: await sha256ResourcePublicationFile(contentPath)
        },
        provenance: {
          generator: "bible-lexicon-maker",
          sourceVersion: options.sourceVersion,
      ...
      if (contentStat.size <= 0)
        throw new Error("timeline-publication-content-empty");
      return { outputDir, manifest };
    },
    validate: validateTimelineResourcePublication
  });
};

export const validateTimelineResourcePublication = async (
  bundleDir: string
) => {
  ...
  if (
    ...
    manifest.identity.resourceId !== "TIMELINE" ||
    !isLanguage(manifest.identity.language) ||
    manifest.offlineArtifact.entry !== "bible-timeline-events.json"
  )
    throw new Error("timeline-publication-manifest-invalid");
  ...
  if (
    ...
    deriveRevision(canonical) !== manifest.revision ||
    canonical.sourceVersion !== manifest.provenance.sourceVersion ||
    canonical.sourceSha256 !== manifest.provenance.sourceSha256 ||
    JSON.stringify(counts(canonical.events)) !== JSON.stringify(manifest.counts)
  )
    throw new Error("timeline-publication-declaration-mismatch");
  ...
  try {
    ...
    const events = Array.isArray(content)
      ? content.map(normalizeEvent).sort((a, b) => a.slug.localeCompare(b.slug))
      : [];
    if (
      sha256(JSON.stringify(content) + "\n") !==
        manifest.offlineArtifact.contentSha256 ||
      JSON.stringify(events) !== JSON.stringify(canonical.events)
    )
      throw new Error("timeline-publication-offline-mismatch");
  } finally {
...
const main = async () => {
  ...
  if (command === "validate") {
    console.log(
      JSON.stringify(
        await validateTimelineResourcePublication(args["--bundle"]),
        null,
        2
      )
    );
  ...
  ) as Omit<TimelineMetadata, "language">;
  const result = await buildTimelineResourcePublication({
    ...metadata,
  ...
  console.log(
    JSON.stringify(
      {
        language: result.manifest.identity.language,
