...
describe('world arrival', () => {
  ...
  it.each([1.75, 2])('approaches zoom %s smoothly and releases camera controls after one second', target => {
    expect(arrivalZoom(1000, 1600, 0.2, target)).toBeCloseTo(0.2 + (target - 0.2) * 0.028)
    expect(arrivalZoom(1000, 2000, 0.2, target)).toBeCloseTo((0.2 + target) / 2)
    expect(arrivalZoom(1000, 2499, 0.2, target)).toBeCloseTo(target)
    expect(arrivalZoom(1000, 2500, 0.2, target)).toBeUndefined()
