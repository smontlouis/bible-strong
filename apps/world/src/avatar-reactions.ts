...
export function loadReactions(scene: Phaser.Scene) {
  for (const activity of ACTIVITIES)
    scene.load.svg(`activity-${activity}`, `data:image/svg+xml;base64,${btoa(activityBadgeSvg(activity))}`)
...
export class AvatarReactions {
  ...
  private visuals = new Map<
    string,
    { body: Phaser.GameObjects.Image; detail: Phaser.GameObjects.Image; startedAt: number; angle: number }
  >()
  ...
  update(
    ...
    for (const [id, activity] of activities) {
      ...
      if (!badge) {
        badge = { image: this.scene.add.image(0, 0, `activity-${activity.activity}`).setDepth(4100), startedAt: now }
        this.badges.set(id, badge)
      }
      const t = reducedMotion ? 1 : Math.min(1, (now - badge.startedAt) / 220)
      const pop = reducedMotion ? 1 : 1 + 2.70158 * (t - 1) ** 3 + 1.70158 * (t - 1) ** 2
      const pulse = reducedMotion ? 1 : 1 + 0.04 * (1 - Math.cos(Math.max(0, now - badge.startedAt - 220) * Math.PI * 2 / 2400)) / 2
      badge.image.setTexture(`activity-${activity.activity}`)
