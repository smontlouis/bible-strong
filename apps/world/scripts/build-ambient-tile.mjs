...
if (!['fish', 'duck'].includes(kind)) throw Error('Expected fish or duck')
...
const crop = kind === 'fish' ? { left: 50, top: 95, width: 150, height: 105 } : {left:75,top:100,width:125,height:75}
const sprites = []
for (let frame = 1; frame <= 120; frame++) {
  ...
  for (let y = 0; y < crop.height; y++) for (let x = 0; x < crop.width; x++) {
    ...
    const coastDistance = kind === 'duck' ? (y + crop.top - (100 + Math.max(0, x + crop.left - 135) * .4)) : (243 - .42 * (x + crop.left) - (y + crop.top)) / 1.09
    const feather = Math.max(0, Math.min(1, Math.min(edge, coastDistance) / 16))
...
const manifest = {id:`${kind}-tile`,x:(kind === 'fish' ? 1151.5 : 1023.5)+crop.left/2,y:(kind === 'fish' ? 383.5 : 0)+crop.top*(kind === 'fish' ? .5 : 128.5/258),width:crop.width/2,height:crop.height*(kind === 'fish' ? .5 : 128.5/258),frameCount:120,frameRate:12,sourceTile:kind === 'fish' ? '4/9-3' : '4/8-0',depth:-79}
