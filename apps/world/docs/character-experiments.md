...
### Themes heart medallion mask repair

The original segmentation kept the heart icon but dropped its surrounding disk on frames 50–95 (one-based), leaving a transparent ring. The offline `sprite/repair-heart.mjs` now tracks the illustrated concentric rim against source-video colors and restores its enclosed disk only. It preserves the original mask in `mask-before-heart-fix.mp4` and writes `mask-heart-fixed.mkv`; rebuilt runtime atlases retain identical placement and frame count. `heart-repair-comparison.png` compares source, previous mask and repaired mask. A shipped-asset regression test samples the cream disk at four affected poses. Rebuild uses `build-reader-atlas.mjs` with the original input video, corrected lossless mask and existing atlas config.
