...
import { FRENCH_INTERNAL_PILOT_SCHEMA_VERSION, FRENCH_INTERNAL_PROPOSER_VIEW_SCHEMA_VERSION, FRENCH_INTERNAL_WORK_POLICY_VERSION, assertProposerABlindView, frenchInternalViewHash, hashFrenchInternalWorkJson } from "../src/lexiconV3/frenchInternalWork.js";
import { validateFrenchPacket } from "../src/lexiconV3/frenchPackets.js";
export const FRENCH_CODEX_PILOT_BATCH_MANIFEST_SCHEMA_VERSION = "lexicon-v3-french-codex-pilot-batches@3";
...
export function buildLexiconV3FrenchCodexPilotBatches(options) {
...
function buildLexiconV3FrenchCodexPilotBatchesUnlocked(options) {
    ...
    const items = pilot.selections.map((selection) => {
        const a = proposerA.get(selection.entryKey);
        const b = proposerB.get(selection.entryKey);
        const packet = packets.get(selection.entryKey);
        if (!packet) {
            throw new Error(`french-codex-pilot-packet-missing:${selection.entryKey}`);
        }
        if (selection.proposerAViewHash !== a.viewHash ||
            selection.proposerBViewHash !== b.viewHash ||
            selection.workViewHash !== a.workViewHash ||
            selection.workViewHash !== b.workViewHash) {
            throw new Error(`french-codex-pilot-selection-stale:${selection.entryKey}`);
        }
        assertExactPilotItemLineage(pilot, selection, a, b, packet);
        if (a.lineage.packetHash !== packet.packetHash ||
            ...
            throw new Error(`french-codex-pilot-packet-lineage-stale:${selection.entryKey}`);
        }
        return {
    ...
    try {
        ...
        const selectedPackets = writeSelectedPacketsArtifact(join(tempDir, "selected-packets.jsonl"), join(resolve(options.outputDir), "selected-packets.jsonl"), selectedPacketsText, items.map((item) => item.packet), {
        ...
        const counts = Object.fromEntries(SIZE_ORDER.map((size) => [
            size,
...
function makeBatches(items, options) {
    const result = [];
    let current = [];
    let bytes = 0;
    for (const item of items) {
        const itemBytes = Buffer.byteLength(item.aLine) + Buffer.byteLength(item.bLine);
...
function writeBatch(tempDir, items, globalIndex) {
    ...
    const packetInput = writeArtifact(join(batchDir, "packets.jsonl"), packetText);
    const aSchema = writeArtifact(join(batchDir, "proposer-a-output.schema.json"), `${JSON.stringify(draftOutputSchema("proposerA", items), null, 2)}\n`);
    const bSchema = writeArtifact(join(batchDir, "proposer-b-output.schema.json"), `${JSON.stringify(draftOutputSchema("proposerB", items), null, 2)}\n`);
    const content = {
        batchId,
        meaningSize,
...
function draftOutputSchema(role, items) {
    ...
    return frenchCodexPilotDraftOutputSchema(role, keys, hashes, segmentIds, items.length);
}
export function frenchCodexPilotDraftOutputSchema(role, keys, hashes, segmentIds, itemCount) {
    return {
...
function readPackets(text, selected) {
    ...
    for (const [index, line] of text.split(/\r?\n/u).entries()) {
        ...
        if (issues.length > 0) {
            throw new Error(`french-codex-pilot-invalid-packet:${index + 1}:${issues.join(",")}`);
        }
        ...
        if (allStepIds.has(packet.identity.stepEntryId)) {
            throw new Error(`french-codex-pilot-duplicate-step-id:${packet.identity.stepEntryId}`);
        }
...
function assertPilot(pilot) {
    const { contentHash, ...content } = pilot;
    if (pilot.schemaVersion !== FRENCH_INTERNAL_PILOT_SCHEMA_VERSION ||
    ...
    for (const [index, selection] of pilot.selections.entries()) {
        ...
        if (pilot.keys[index] !== selection.entryKey ||
            !SHA256_PATTERN.test(selectionHash) ||
            hashFrenchInternalWorkJson(selectionContent) !== selectionHash) {
            throw new Error(`french-codex-pilot-selection-invalid:${selection.entryKey}`);
        }
    }
}
function assertExactPilotItemLineage(pilot, selection, proposerA, proposerB, packet) {
    const expectedLineageHash = hashFrenchInternalJson(selection.lineage);
    if (hashFrenchInternalJson(proposerA.lineage) !== expectedLineageHash ||
        ...
        throw new Error(`french-codex-pilot-exact-lineage-mismatch:${selection.entryKey}`);
    }
}
function assertExactPilotCoverage(pilot, proposerA, proposerB) {
...
export function assertFrenchCodexPilotBatchManifest(manifest, options = {}) {
    if (manifest.schemaVersion !== FRENCH_CODEX_PILOT_BATCH_MANIFEST_SCHEMA_VERSION) {
        throw new Error(`french-codex-pilot-manifest-migration-required:${String(manifest.schemaVersion)}`);
    }
    const normalizedOptions = typeof options === "boolean" ? { verifyFiles: options } : options;
    ...
    const keys = new Set();
    const orderedKeys = [];
    const byMeaningSize = Object.fromEntries(SIZE_ORDER.map((size) => [size, 0]));
    for (const batch of manifest.batches) {
        const { batchHash, ...batchContent } = batch;
        if (!/^pilot-(short|medium|long|very_long)-\d{3}$/u.test(batch.batchId) ||
        ...
        if (normalizedOptions.verifyFiles) {
            ...
            if (dirname(batch.inputs.proposerB.path) !== directory ||
                ...
                throw new Error(`french-codex-pilot-batch-path-invalid:${batch.batchId}`);
            }
        }
    }
    if (keys.size !== manifest.counts.entries ||
        SIZE_ORDER.some((size) => byMeaningSize[size] !== manifest.counts.byMeaningSize[size]) ||
        Object.values(manifest.counts.byMeaningSize).reduce((sum, count) => sum + count, 0) !== manifest.counts.entries) {
        throw new Error("french-codex-pilot-manifest-coverage-invalid");
    }
    const batchRoot = dirname(dirname(manifest.batches[0].inputs.proposerA.path));
    assertSelectedPacketsMetadata(manifest.selectedPackets, join(batchRoot, "selected-packets.jsonl"), manifest.counts.entries, manifest.lineage);
    if (!normalizedOptions.verifyFiles)
        return;
    ...
    const pilot = JSON.parse(readFileSync(manifest.sourcePaths.pilot, "utf8"));
    assertPilot(pilot);
    if (pilot.contentHash !== manifest.sourceDigests.pilotContentHash ||
    ...
    const sourceA = readViews(readFileSync(manifest.sourcePaths.proposerA, "utf8"), "proposerA");
    const sourceB = readViews(readFileSync(manifest.sourcePaths.proposerB, "utf8"), "proposerB");
    const sourcePackets = readPackets(readFileSync(manifest.sourcePaths.packets, "utf8"), keys);
    if (sourceA.size !== keys.size ||
        sourceB.size !== keys.size ||
        sourcePackets.size !== keys.size) {
        throw new Error("french-codex-pilot-source-coverage-mismatch");
    }
    const selectedPackets = readPacketSequence(manifest.selectedPackets.path, "french-codex-pilot-selected-packets");
    assertSelectedPacketsContent(manifest.selectedPackets, selectedPackets, orderedKeys, manifest.batches);
    if (selectedPackets.some((packet) => sourcePackets.get(packet.entryKey)?.packetHash !== packet.packetHash ||
    ...
    for (const batch of manifest.batches) {
        for (const [index, key] of batch.keys.entries()) {
            if (sourceA.get(key)?.viewHash !== batch.proposerAViewHashes[index] ||
...
function assertManifestArtifact(artifact, batchId) {
...
function writeSelectedPacketsArtifact(physicalPath, finalPath, text, packets, lineage) {
...
export function frenchCodexSelectedPacketsLogicalDigest(packets) {
    return hashFrenchInternalJson(packets.map((packet) => ({
        entryKey: packet.entryKey,
        packetHash: packet.packetHash
    })));
}
export function assertSelectedPacketsMetadata(artifact, expectedPath, expectedRecords, expectedLineage) {
...
function assertSelectedPacketsContent(artifact, packets, expectedKeys, batches) {
    ...
    if (sha256(text) !== artifact.sha256 ||
        ...
        packets.some((packet) => packet.englishRelease.releaseKey !== artifact.releaseKey ||
            packet.englishRelease.releaseSnapshotFingerprint !==
                artifact.releaseSnapshotFingerprint) ||
...
function readPacketSequence(path, label) {
    if (!existsSync(path))
...
function installDirectoryAtomically(tempDir, outputDir, backupDir) {
    ...
    try {
        ...
        if (existed && existsSync(backupDir))
...
function rewriteBatchPaths(batch, oldRoot, newRoot) {
    const rewritten = JSON.parse(JSON.stringify(batch).split(oldRoot).join(newRoot));
    const { batchHash: _batchHash, ...content } = rewritten;
    void _batchHash;
    return { ...content, batchHash: hashFrenchInternalJson(content) };
}
function assertOptions(options) {
    if (!Number.isSafeInteger(options.maxCombinedBytes) ||
        options.maxCombinedBytes < 1) {
        throw new Error("french-codex-pilot-invalid-max-bytes");
    }
    for (const size of SIZE_ORDER) {
        if (!Number.isSafeInteger(options.maxItems[size]) ||
...
export function parseFrenchCodexPilotBatchArgs(args) {
    ...
    return {
        pilotPath: resolve(values.get("pilot") ?? join(DEFAULT_WORK_DIR, "pilot-keys.json")),
        proposerAPath: resolve(values.get("proposer-a") ??
            join(DEFAULT_WORK_DIR, "pilot-proposer-a-input.jsonl")),
        proposerBPath: resolve(values.get("proposer-b") ??
...
if (import.meta.url === pathToFileURL(resolve(process.argv[1] ?? "")).href) {
    try {
        ...
        process.stderr.write(`${basename(process.argv[1] ?? "buildLexiconV3FrenchCodexPilotBatches")}: ${error instanceof Error ? error.message : String(error)}\n`);
        process.exitCode = 1;
    }
}
