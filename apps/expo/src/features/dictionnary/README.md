...
`DictionaryDetailTabScreen` conserve l’identité exacte de l’article (`work`, `resourceId`, `language`, `entryId`). Lorsqu’une correspondance existe, un sélecteur montre les autres dictionnaires et variantes linguistiques. Le sélecteur « Aussi dans… » met à jour le state Jotai ; sur une page publique, son adaptateur remplace aussi l’URL canonique sans ajouter d’écran à la pile. Les liens `bible://`, `strong://` et les liens vers d’autres articles ouvrent leur surface native.
