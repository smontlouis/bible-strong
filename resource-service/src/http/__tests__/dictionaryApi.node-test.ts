...
describe('v1 Dictionary API', () => {
  it('exposes keyset page tokens and accepts them on the following page', async () => {
    ...
    try {
      const first = await web.handler(
        new Request('http://localhost/v1/dictionaries/fr/entries?initial=a&limit=1')
      )
      ...
      const next = await web.handler(
        new Request(
          `http://localhost/v1/dictionaries/fr/entries?initial=a&limit=1&cursor=${encodeURIComponent(body.nextCursor)}`
        )
      )
  ...
  it('returns several definitions from one batch endpoint', async () => {
    ...
    try {
      const response = await web.handler(
        new Request('http://localhost/v1/dictionaries/fr/entries/batch?words=amour%2Cange')
      )
  ...
  it('rejects malformed page cursors', async () => {
    ...
    try {
      const response = await web.handler(
        new Request('http://localhost/v1/dictionaries/fr/entries?cursor=not-a-cursor')
      )
