...
var bcv_matcher = class {
  ...
  has_potential_reference(s) {
    if (this.parent.options.book_alone_strategy !== "ignore" || this.parent.options.book_range_strategy !== "ignore" || this.parent.options.book_sequence_strategy !== "ignore") {
      return true;
    }
    if (/\d/.test(s)) {
      return true;
    }
    if (this.parent.options.non_latin_digits_strategy === "replace" && this.replace_non_ascii_numbers(s) !== s) {
      return true;
    }
    return false;
  }
  ...
  match_books(s) {
    const books = [];
    const start_chars = this.get_start_chars(s);
    for (const book of this.parent.regexps.books) {
      if (book.starts && this.has_required_start(start_chars, book.starts) === false) {
        continue;
      }
      let has_replacement = false;
      s = s.replace(book.regexp, (_full, bk) => {
        if (this.is_strict_book_match(bk) === false) {
          return _full;
        }
  ...
  has_required_start(start_chars, starts) {
    for (const char of starts) {
      if (start_chars.has(char)) {
        return true;
      }
    }
    return false;
  }
  is_strict_book_match(match) {
    if (this.parent.options.book_match_strategy !== "strict") {
      return true;
    }
    if (match.length > 2 || /[^\p{L}]/u.test(match)) {
      return true;
    }
...
var bcv_options = class {
  constructor(parent) {
    ...
    this.book_match_strategy = "default";
...
const bcv = new bcv_parser(lang);`;
  ...
  parse(string_to_parse) {
    ...
    if (this.matcher.has_potential_reference(string_to_parse) === false) {
      return this;
    }
